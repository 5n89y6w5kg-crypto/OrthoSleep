      ...
      <div>
        ...
        ${r.costForWidth > 0 ? `
          <div style="margin-top:6px;font-size:11.5px;color:var(--text-dim);">
            Себест: <b>${fmtMoney(r.costForWidth)}</b>
            ${r.marginPct !== null ? ` · Прибыль: <b style="color:var(--accent);">${fmtMoney(r.margin)} (${r.marginPct}%)</b>` : ''}
          </div>
        ` : ''}
      </div>
      <div style="text-align:right;">
...
async function downloadOrdersCsv() {
  try {
    const { data: orders, error } = await sb.from('orders')
      .select('*, clients(full_name,phone), deals(title,deal_items(qty,price,mattress_width,mattress_length,products(name)))')
      .order('created_at', { ascending: false });
    if (error) throw error;

    const headers = ['Номер заказа','Дата','Клиент','Телефон','Товар','Размер','Сумма (сом)','Оплачено (сом)','Остаток (сом)','Статус оплаты','Статус доставки','Адрес доставки'];
    const rows = (orders||[]).map(o => {
      ...
      const paid = o.payment_status==='paid'?'Оплачен':o.payment_status==='partial'?'Частично':'Не оплачен';
      const delivery = o.delivery_status==='delivered'?'Доставлен':o.delivery_status==='shipped'?'В пути':o.delivery_status==='preparing'?'Готовится':'Ожидает';
      return [
        o.order_number||o.id,
...
async function exportFinanceCSV() {
  ...
  const { data: orders } = await sb.from('orders')
    .select('*, clients(full_name,phone), deals(deal_items(qty,price,mattress_width,mattress_length,cost_price_at_sale,products(name)))')
    .gte('created_at', periodStart.toISOString())
    .lte('created_at', periodEnd.toISOString())
    .order('created_at', { ascending: false });

  const rows = [['Номер заказа','Дата','Клиент','Телефон','Товар','Размер','Сумма','Оплачено','Статус оплаты','Себестоимость','Прибыль','Статус поставщика']];

  (orders || []).forEach(o => {
    const item = o.deals?.deal_items?.[0];
    const product = item?.products?.name || '';
    const size = item?.mattress_width ? `${item.mattress_width}×${item.mattress_length||200}` : '';
  ...
  const csvContent = ['sep=;', rows[0].join(';'), ...rows.slice(1).map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(';'))].join('\r\n');
  const blob = new Blob(['\uFEFF' + csvContent], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `finance_${periodStart.toISOString().split('T')[0]}_${periodEnd.toISOString().split('T')[0]}.csv`;
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  URL.revokeObjectURL(url);
