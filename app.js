    ...
    <div class="sheet-body">
      ...
      <div class="card" style="background:${order.cost_paid ? 'rgba(0,185,86,0.08)' : 'rgba(232,162,58,0.08)'};border-color:${order.cost_paid ? 'var(--accent)' : 'var(--orange)'};">
        ...
        <div class="field"><label>Модель матраса</label>
          <select id="order-mattress-model" onchange="recalcOrderCost(${orderId})">
            ...
            ${(await sb.from('products').select('id,name,cost_per_cm,cost_price').eq('is_active', true)).data?.map(p =>
              `<option value="${p.id}" data-cpc="${p.cost_per_cm || (p.cost_price ? (p.cost_price/180).toFixed(4) : 0)}" ${order.mattress_model == p.id ? 'selected' : ''}>${escapeHtml(p.name)}</option>`
            ).join('') || ''}
          </select>
        </div>
        <div style="display:flex;gap:10px;">
          <div class="field" style="flex:1;"><label>Ширина (см)</label>
            <input type="number" id="order-mattress-width" value="${order.mattress_width || 180}" oninput="recalcOrderCost(${orderId})">
          </div>
          <div class="field" style="flex:1;"><label>Длина (см)</label>
            <input type="number" id="order-mattress-length" value="${order.mattress_length || 200}" oninput="recalcOrderCost(${orderId})">
          </div>
        </div>
        ...
        <div class="field"><label>Сумма себестоимости (сом) — можно изменить вручную</label>
          <input type="number" id="order-cost-amount" value="${order.cost_paid_amount || ''}" placeholder="Авторасчёт или введи вручную">
        </div>
...
async function loadFinanceContent(days) {
  ...
  const { data: orders, error } = await sb
    .from('orders')
    .select('*, clients(full_name,phone), deals(id,title,deal_items(qty,price,products(name,cost_price)))')
  ...
  const rows = (orders || []).map(o => {
    ...
    // Себестоимость: берём из поля cost_paid_amount если оно заполнено,
    // иначе из deal_items (товары × закупочная цена)
    const costFromField = Number(o.cost_paid_amount) || 0;
    const costItems = o.deals?.deal_items || [];
    const costFromDeal = costItems.reduce((s, i) => s + (i.qty * Number(i.products?.cost_price || 0)), 0);
    const cost = costFromField > 0 ? costFromField : costFromDeal;
    ...
    return { ...o, revenue, received, cost, costPaid, profit };
  });
        ...
        <div class="card" style="font-size:13px;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
            ...
            <div style="text-align:right;">
              ...
              <div style="font-size:11px;color:${o.payment_status==='paid'?'var(--accent)':'var(--orange)'};">${o.payment_status==='paid'?'✅ Оплачен':'⏳ Не оплачен'}</div>
            </div>
          </div>
          <div style="display:flex;justify-content:space-between;color:var(--text-dim);font-size:11.5px;">
            <span>Себест: ${fmtMoney(o.cost)}</span>
            <span>Прибыль: <span style="color:var(--accent);font-weight:600;">${fmtMoney(o.profit)}</span></span>
            <span>${o.cost_paid ? `✅ Оплачено поставщику` : `⚠️ Долг: ${fmtMoney(o.cost)}`}</span>
          </div>
          ${!o.cost_paid && o.cost > 0 ? `
            <button class="btn-secondary" style="margin-top:8px;font-size:11.5px;padding:5px 10px;" onclick="markCostPaid(${o.id},${o.cost})">Отметить оплату поставщику</button>
          ` : ''}
        </div>
