...
function renderTopbar() {
  ...
  const showBack = ['client_detail', 'deal_detail', 'tasks', 'analytics'].includes(S.route);
  ...
  return `
    <div class="page">
      <div class="card" style="text-align:center;padding:22px 16px;">
        <div class="avatar" style="width:60px;height:60px;font-size:22px;margin:0 auto 10px;">${initials(client.full_name)}</div>
...
async function openMessagesSheet(clientId) {
  ...
  const client = (await sb.from('clients').select('full_name').eq('id', clientId).single()).data;
