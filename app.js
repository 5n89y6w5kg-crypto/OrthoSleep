...
function renderTopbar() {
  const titles = {
    ...
    finance: ['Финансы', 'Себестоимость · Прибыль · Долги'],
    conversation_analysis: ['Анализ разговоров', 'AI анализирует переписки'],
    inbox: ['Сообщения', 'Instagram · WhatsApp · Messenger'],
    ai_knowledge: ['Акции и информация', 'Что знает AI о бизнесе'],
    templates: ['Шаблоны сообщений', 'Быстрые ответы для менеджера'],
    health: ['Статус системы', 'Мониторинг и здоровье'],
    ai_costs: ['Расходы AI', 'Токены и стоимость'],
    dialog_export: ['Анализ диалогов', 'Экспорт для Claude'],
    managers: ['Менеджеры', 'Продажи и бонусы'],
    manager_detail: ['Менеджер', 'Детальный отчёт'],
    activity_logs: ['Логи активности', 'AI · Менеджеры · Система'],
    notifications: ['Уведомления', ''],
    deal_detail: ['Сделка', ''],
  };
  const [title, sub] = titles[S.route] || ['OrthoSleep', ''];
  const showBack = ['client_detail', 'deal_detail', 'tasks', 'analytics', 'inbox', 'ai_knowledge', 'activity_logs', 'notifications', 'finance', 'conversation_analysis', 'templates', 'health', 'ai_costs', 'dialog_export', 'managers', 'manager_detail'].includes(S.route);
...
async function loadRouteContent() {
  ...
  try {
    ...
    switch (S.route) {
      ...
      case 'finance': html = await renderFinanceReport(); break;
      case 'conversation_analysis': html = await renderConversationAnalysis(); break;
      ...
      case 'health': html = await renderHealthCheck(); break;
      case 'ai_costs': html = await renderAiCosts(); break;
      case 'dialog_export': html = await renderDialogExport(); break;
      case 'managers': html = await renderManagers(); break;
      case 'manager_detail': html = await renderManagerDetail(S.routeParams?.id); break;
...
async function renderClients() {
  ...
  return `
    <div class="page" style="padding-bottom:8px;">
      <input type="text" placeholder="🔍 Поиск по имени или телефону" oninput="filterClientsList(this.value)" style="margin-bottom:10px;">
      <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:10px;scrollbar-width:none;">
        <button class="tab ${_clientFilter==='all'?'active':''}" style="white-space:nowrap;" onclick="setClientFilter('all')">Все</button>
        <button class="tab ${_clientFilter==='hot'?'active':''}" style="white-space:nowrap;" onclick="setClientFilter('hot')">🔴 Горячие</button>
        <button class="tab ${_clientFilter==='warm'?'active':''}" style="white-space:nowrap;" onclick="setClientFilter('warm')">🟡 Тёплые</button>
        <button class="tab ${_clientFilter==='cold'?'active':''}" style="white-space:nowrap;" onclick="setClientFilter('cold')">❄️ Холодные</button>
        <button class="tab ${_clientFilter==='no_deal'?'active':''}" style="white-space:nowrap;" onclick="setClientFilter('no_deal')">Без сделки</button>
        <button class="tab ${_clientFilter==='human'?'active':''}" style="white-space:nowrap;" onclick="setClientFilter('human')">👤 У менеджера</button>
      </div>
      <div id="clients-list-container">
        ${renderClientsListItems(applyClientFilter(clients))}
      </div>
    </div>
  `;
}

function applyClientFilter(clients) {
  if (_clientFilter === 'all') return clients;
  if (_clientFilter === 'hot') return clients.filter(c => c.lead_status === 'hot');
  if (_clientFilter === 'warm') return clients.filter(c => c.lead_status === 'warm');
  if (_clientFilter === 'cold') return clients.filter(c => c.lead_status === 'cold');
  if (_clientFilter === 'no_deal') return clients.filter(c => !c.has_deal); // упрощённо
  if (_clientFilter === 'human') return clients.filter(c => c.conversation_owner === 'human');
  return clients;
}

function setClientFilter(filter) {
  _clientFilter = filter;
  const filtered = applyClientFilter(S.cache.clientsList || []);
  document.getElementById('clients-list-container').innerHTML = renderClientsListItems(filtered);
  // Обновляем активный таб
  document.querySelectorAll('.tab').forEach(t => {
    const f = t.getAttribute('onclick')?.match(/'(\w+)'/)?.[1];
    if (f) t.classList.toggle('active', f === filter);
  });
}
...
function renderClientsListItems(clients) {
  ...
  const statusIcon = { hot: '🔴', warm: '🟡', cold: '❄️' };
    ...
    <div class="client-row" onclick="navigate('client_detail',{id:${c.id}})">
      ...
      <div class="info">
        <div class="name" style="display:flex;align-items:center;gap:5px;">
          ...
          ${c.lead_status ? `<span style="font-size:12px;">${statusIcon[c.lead_status]||''}</span>` : ''}
          ${c.conversation_owner==='human' ? `<span style="font-size:10px;background:rgba(232,162,58,0.15);color:var(--orange);padding:1px 5px;border-radius:6px;">👤</span>` : ''}
        </div>
        <div class="meta">
...
async function renderClientDetail(id) {
  ...
  // Загружаем историю заказов после рендера (не блокируем основной рендер)
  setTimeout(() => loadClientOrders(client.id), 100);
  ...
  return `
    <div class="page">
      ...
      <div class="card" id="client-orders-${client.id}">
        <div style="color:var(--text-faint);font-size:12.5px;">Загрузка...</div>
      </div>

      <div class="section-title">Статус лида</div>
      <div class="card">
        <div style="display:flex;gap:8px;">
          ${['hot','warm','cold'].map(s => {
            const labels = {hot:'🔴 Горячий', warm:'🟡 Тёплый', cold:'❄️ Холодный'};
            const active = client.lead_status === s;
            return `<button onclick="setLeadStatus(${client.id},'${s}')" style="flex:1;padding:8px;border-radius:8px;font-size:12px;font-weight:${active?'700':'400'};background:${active?'var(--violet)':'var(--bg-elevated)'};color:${active?'#fff':'var(--text-dim)'};border:none;">${labels[s]}</button>`;
          }).join('')}
        </div>
      </div>
...
async function loadClientOrders(clientId) {
  const el = document.getElementById(`client-orders-${clientId}`);
  if (!el) return;
  el.innerHTML = await renderClientOrderHistory(clientId);
}

async function setLeadStatus(clientId, status) {
  const { error } = await sb.from('clients').update({ lead_status: status }).eq('id', clientId);
...
function renderDealItemsList(items) {
  ...
  return items.map(it => {
    const w = it.mattress_width || it.products?.base_width || 180;
    const l = it.mattress_length || it.products?.base_length || 200;
    const sizeStr = `${w}×${l}`;
    return `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border);">
      <div>
        <div style="font-weight:600;font-size:13.5px;">${escapeHtml(it.products?.name || '')} (${sizeStr})</div>
        <div style="font-size:12px;color:var(--text-dim);">${it.qty} × ${fmtMoney(it.price)}${it.cost_price_at_sale ? ` · себест: ${fmtMoney(it.cost_price_at_sale)}` : ''}</div>
      </div>
...
function openAddProductToDealSheet() {
  const products = (S.cache.allProducts || []).filter(p => p.is_active !== false);
  openSheet(`
    <div class="sheet-header"><h3>Добавить товар</h3><button class="close-x" onclick="closeSheet()">✕</button></div>
    <div class="sheet-body">
      <div id="add-product-form">
        <div class="field"><label>Модель матраса</label>
          <select id="add-item-product" onchange="onAddItemProductChange()">
            <option value="">— выбери модель —</option>
            ${products.map(p => `<option value="${p.id}"
              data-price="${p.sale_price}"
              data-ppc="${p.price_per_cm || 0}"
              data-cpc="${p.cost_per_cm || (p.cost_price ? (p.cost_price/180).toFixed(4) : 0)}"
              data-name="${escapeHtml(p.name).replace(/"/g,'&quot;')}">${escapeHtml(p.name)}</option>`).join('')}
          </select>
        </div>
        <div class="calc-size-row" style="margin:10px 0;">
          <input type="number" id="add-item-width" value="180" placeholder="Ширина" oninput="onAddItemSizeChange()" style="text-align:center;">
          <span class="x">×</span>
          <input type="number" id="add-item-length" value="200" placeholder="Длина" oninput="onAddItemSizeChange()" style="text-align:center;">
        </div>
        <div style="font-size:11px;color:var(--text-faint);margin-bottom:10px;">Ширина × Длина в сантиметрах</div>
        <div id="add-item-calc-result" style="font-size:12.5px;color:var(--text-dim);margin-bottom:10px;"></div>
        <div class="field"><label>Цена продажи (сом) — можно изменить</label>
          <input type="number" id="add-item-price" placeholder="0">
        </div>
    ...
    <div class="sheet-footer">
      <button class="btn-primary" onclick="addProductToDealWithSize()">Добавить в сделку</button>
    </div>
  `);
}

function onAddItemProductChange() {
  const select = document.getElementById('add-item-product');
  const opt = select.options[select.selectedIndex];
  onAddItemSizeChange();
}

function onAddItemSizeChange() {
  const select = document.getElementById('add-item-product');
  const opt = select?.options[select.selectedIndex];
  if (!opt?.value) return;

  const ppc = parseFloat(opt.dataset.ppc) || 0;
  const cpc = parseFloat(opt.dataset.cpc) || 0;
  const width = parseInt(document.getElementById('add-item-width').value) || 180;
  const length = parseInt(document.getElementById('add-item-length').value) || 200;
  const priceInput = document.getElementById('add-item-price');
  const resultDiv = document.getElementById('add-item-calc-result');

  if (ppc > 0) {
    const calcPrice = Math.round(ppc * width);
    const calcCost = cpc > 0 ? Math.round(cpc * width) : 0;
    const profit = calcCost > 0 ? calcPrice - calcCost : null;

    resultDiv.innerHTML = `Цена: <b>${fmtMoney(calcPrice)}</b>${calcCost > 0 ? ` · Себест: <b>${fmtMoney(calcCost)}</b>` : ''}${profit !== null ? ` · Прибыль: <b style="color:var(--accent)">${fmtMoney(profit)}</b>` : ''}`;
    if (!priceInput.value) priceInput.value = calcPrice;
    else priceInput.value = calcPrice; // всегда обновляем при смене размера
  } else {
    const fixedPrice = parseFloat(opt.dataset.price) || 0;
    if (fixedPrice && !priceInput.value) priceInput.value = fixedPrice;
  }
}

async function addProductToDealWithSize() {
  const select = document.getElementById('add-item-product');
  const productId = parseInt(select.value);
  if (!productId) { showToast('Выбери модель матраса', 'error'); return; }

  const opt = select.options[select.selectedIndex];
  const name = opt.dataset.name || escapeHtml(opt.text);
  ...
  const length = parseInt(document.getElementById('add-item-length').value) || 200;
  const cpc = parseFloat(opt.dataset.cpc) || 0;
  const costAtSale = cpc > 0 ? Math.round(cpc * width) : null;

  if (!price) { showToast('Укажи цену продажи', 'error'); return; }

  closeSheet();
  await withLoading(async () => {
    const dealId = S.cache.currentDeal.id;
    const { error } = await sb.from('deal_items').insert({
      deal_id: dealId,
        ...
        <div class="product-card">
          ...
          <div class="prow">
            <div>
              ...
              ${p.cost_price ? `<div style="font-size:11px;color:var(--text-faint);">Себест: ${fmtMoney(p.cost_price)}</div>` : `<div style="font-size:11px;color:var(--orange);">⚠️ Нет себестоимости</div>`}
              ${p.photo_url ? `<div style="font-size:11px;color:var(--accent);">📷 Фото есть</div>` : `<div style="font-size:11px;color:var(--text-faint);">Нет фото</div>`}
            </div>
...
function openCreateProductSheet() {
  ...
  openSheet(`
    ...
    <div class="sheet-body">
      ...
      <div class="field"><label>URL фото матраса (для отправки клиентам в Instagram)</label><input type="url" id="new-prod-photo" placeholder="https://... ссылка на фото"></div>
      <div class="field"><label>Себестоимость за 1 см ширины</label><input type="number" id="new-prod-costpercm" step="0.0001" placeholder="напр. 8.89 (= 1600 / 180)"></div>
...
async function submitCreateProduct() {
  ...
  await withLoading(async () => {
    ...
    const payload = {
      ...
      photo_url: document.getElementById('new-prod-photo').value.trim() || null,
      cost_per_cm: parseFloat(document.getElementById('new-prod-costpercm').value) || null,
    };
...
function openEditProductSheet(id) {
  ...
  openSheet(`
    ...
    <div class="sheet-body">
      ...
      <div class="field"><label>URL фото матраса (для отправки клиентам в Instagram)</label><input type="url" id="edit-prod-photo" value="${escapeHtml(p.photo_url || '')}"></div>
      <div class="field"><label>Себестоимость за 1 см ширины (авторасчёт: ${p.cost_price && p.cost_price > 0 ? (p.cost_price/180).toFixed(4) : '—'} сом/см)</label><input type="number" id="edit-prod-costpercm" step="0.0001" value="${p.cost_per_cm || (p.cost_price ? (p.cost_price/180).toFixed(4) : '')}" placeholder="Себест за 1 см = закупочная / 180"></div>
...
async function submitEditProduct(id) {
  await withLoading(async () => {
    const payload = {
      ...
      photo_url: document.getElementById('edit-prod-photo').value.trim() || null,
      cost_per_cm: parseFloat(document.getElementById('edit-prod-costpercm').value) || null,
    };
...
let _ordersFilter = { payment: 'all', delivery: 'all', supplier: 'all', search: '', days: 0, dateFrom: '', dateTo: '' };

async function renderOrders() {
  return await loadOrdersContent();
}

async function loadOrdersContent() {
  const isAdmin = S.profile?.role === 'admin';
  const { data: managers } = isAdmin ? await sb.from('profiles').select('id,full_name').eq('is_active', true) : { data: [] };

  let query = sb.from('orders')
    .select('*, clients(full_name,phone), deals(id,title,assigned_to,profiles:assigned_to(full_name),deal_items(qty,price,mattress_width,mattress_length,products(name)))')
    .order('created_at', { ascending: false });

  if (_ordersFilter.days > 0) {
    const from = new Date();
    from.setDate(from.getDate() - _ordersFilter.days);
    query = query.gte('created_at', from.toISOString());
  } else if (_ordersFilter.dateFrom && _ordersFilter.dateTo) {
    const to = new Date(_ordersFilter.dateTo); to.setHours(23,59,59);
    query = query.gte('created_at', new Date(_ordersFilter.dateFrom).toISOString()).lte('created_at', to.toISOString());
  }
  if (_ordersFilter.payment !== 'all') query = query.eq('payment_status', _ordersFilter.payment);
  if (_ordersFilter.supplier === 'paid') query = query.eq('cost_paid', true);
  if (_ordersFilter.supplier === 'unpaid') query = query.eq('cost_paid', false);

  const { data: orders, error } = await query;
  if (error) throw error;

  // Фильтрация доставки и поиска на клиенте
  let filtered = orders || [];
  if (_ordersFilter.delivery !== 'all') filtered = filtered.filter(o => o.delivery_status === _ordersFilter.delivery);
  if (_ordersFilter.search) {
    const q = _ordersFilter.search.toLowerCase();
  ...
  return `
    <div class="page">
      <input type="text" placeholder="🔍 Поиск по номеру, клиенту, телефону..."
        value="${escapeHtml(_ordersFilter.search)}"
        oninput="_ordersFilter.search=this.value; refreshOrdersList()"
        style="margin-bottom:10px;">

      <div style="overflow-x:auto;white-space:nowrap;padding-bottom:6px;margin-bottom:8px;scrollbar-width:none;">
        <button onclick="setOrderFilter('payment','all')" style="display:inline-block;margin-right:5px;padding:4px 10px;border-radius:14px;font-size:11px;font-weight:600;border:1px solid var(--border);background:${_ordersFilter.payment==='all'?'var(--violet)':'var(--bg-card)'};color:${_ordersFilter.payment==='all'?'#fff':'var(--text-dim)'};">Все</button>
        <button onclick="setOrderFilter('payment','unpaid')" style="display:inline-block;margin-right:5px;padding:4px 10px;border-radius:14px;font-size:11px;font-weight:600;border:1px solid var(--border);background:${_ordersFilter.payment==='unpaid'?'var(--red)':'var(--bg-card)'};color:${_ordersFilter.payment==='unpaid'?'#fff':'var(--text-dim)'};">❌ Не оплачен</button>
        <button onclick="setOrderFilter('payment','partial')" style="display:inline-block;margin-right:5px;padding:4px 10px;border-radius:14px;font-size:11px;font-weight:600;border:1px solid var(--border);background:${_ordersFilter.payment==='partial'?'var(--orange)':'var(--bg-card)'};color:${_ordersFilter.payment==='partial'?'#fff':'var(--text-dim)'};">⚠️ Частично</button>
        <button onclick="setOrderFilter('payment','paid')" style="display:inline-block;margin-right:5px;padding:4px 10px;border-radius:14px;font-size:11px;font-weight:600;border:1px solid var(--border);background:${_ordersFilter.payment==='paid'?'var(--accent)':'var(--bg-card)'};color:${_ordersFilter.payment==='paid'?'#fff':'var(--text-dim)'};">✅ Оплачен</button>
        <button onclick="setOrderFilter('delivery','delivered')" style="display:inline-block;margin-right:5px;padding:4px 10px;border-radius:14px;font-size:11px;font-weight:600;border:1px solid var(--border);background:${_ordersFilter.delivery==='delivered'?'var(--accent)':'var(--bg-card)'};color:${_ordersFilter.delivery==='delivered'?'#fff':'var(--text-dim)'};">🚚 Доставлен</button>
        <button onclick="setOrderFilter('delivery','all')" style="display:inline-block;margin-right:5px;padding:4px 10px;border-radius:14px;font-size:11px;font-weight:600;border:1px solid var(--border);background:${_ordersFilter.delivery==='all'&&_ordersFilter.payment==='all'&&_ordersFilter.supplier==='all'?'':'var(--bg-card)'};color:var(--text-dim);">Сброс</button>
        ${isAdmin ? `<button onclick="setOrderFilter('supplier','unpaid')" style="display:inline-block;margin-right:5px;padding:4px 10px;border-radius:14px;font-size:11px;font-weight:600;border:1px solid var(--border);background:${_ordersFilter.supplier==='unpaid'?'var(--orange)':'var(--bg-card)'};color:${_ordersFilter.supplier==='unpaid'?'#fff':'var(--text-dim)'};">⏳ Поставщик не оплачен</button>` : ''}
      </div>

      <div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap;">
        ${[7,30,90].map(d => `<button onclick="setOrderDays(${d})" style="padding:4px 10px;border-radius:12px;font-size:11px;border:1px solid var(--border);background:${_ordersFilter.days===d?'var(--violet)':'var(--bg-card)'};color:${_ordersFilter.days===d?'#fff':'var(--text-dim)'};">${d} дней</button>`).join('')}
        <button onclick="setOrderDays(0)" style="padding:4px 10px;border-radius:12px;font-size:11px;border:1px solid var(--border);background:${_ordersFilter.days===0&&!_ordersFilter.dateFrom?'var(--violet)':'var(--bg-card)'};color:${_ordersFilter.days===0&&!_ordersFilter.dateFrom?'#fff':'var(--text-dim)'};">Все</button>
        <button onclick="toggleOrderDatePicker()" style="padding:4px 10px;border-radius:12px;font-size:11px;border:1px solid var(--border);background:${_ordersFilter.dateFrom?'var(--violet)':'var(--bg-card)'};color:${_ordersFilter.dateFrom?'#fff':'var(--text-dim)'};">📅 Свой</button>
      </div>

      <div id="order-date-picker" style="display:${_ordersFilter.dateFrom?'flex':'none'};gap:8px;margin-bottom:10px;align-items:center;">
        <input type="date" id="ord-date-from" value="${_ordersFilter.dateFrom}" style="flex:1;" oninput="_ordersFilter.dateFrom=this.value">
        <span>—</span>
        <input type="date" id="ord-date-to" value="${_ordersFilter.dateTo}" style="flex:1;" oninput="_ordersFilter.dateTo=this.value">
        <button class="btn-primary" style="padding:8px 10px;" onclick="applyOrderDateFilter()">OK</button>
      </div>

      <div style="font-size:12px;color:var(--text-dim);margin-bottom:8px;">Найдено: ${filtered.length} заказов</div>

      ${filtered.map(o => {
        const firstItem = o.deals?.deal_items?.[0];
        const productName = firstItem?.products?.name || '';
        const w = firstItem?.mattress_width || '';
        const l = firstItem?.mattress_length || '';
        const sizeStr = w && l ? `${w}×${l}` : '';
        return `
        <div class="card">
          <div onclick="openOrderDetailSheet(${o.id})" style="cursor:pointer;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;">
              <div>
                <div style="font-weight:700;font-size:14px;">${escapeHtml(o.order_number)}</div>
                <div style="font-size:11.5px;color:var(--text-faint);">${fmtDate(o.created_at)}</div>
                <div style="font-size:12.5px;color:var(--text-dim);margin-top:2px;">${escapeHtml(o.clients?.full_name || '')}</div>
                ${productName ? `<div style="font-size:12px;color:var(--text-faint);margin-top:1px;">${escapeHtml(productName)}${sizeStr ? ' · ' + sizeStr + ' см' : ''}</div>` : ''}
              </div>
              <div style="text-align:right;">
                <div style="color:var(--gold);font-weight:700;">${fmtMoney(o.total_amount)}</div>
                ${o.deals?.profiles?.full_name ? `<div style="font-size:11px;color:var(--text-faint);">${escapeHtml(o.deals?.profiles?.full_name)}</div>` : ''}
              </div>
            </div>
            <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;">
              <span class="badge" style="background:${o.payment_status==='paid'?'rgba(0,185,86,0.15)':o.payment_status==='partial'?'rgba(232,162,58,0.15)':'rgba(220,50,50,0.15)'};color:${o.payment_status==='paid'?'var(--accent)':o.payment_status==='partial'?'var(--orange)':'var(--red)'};">${o.payment_status==='paid'?'Оплачен':o.payment_status==='partial'?'Частично':'Не оплачен'}</span>
              <span class="badge" style="background:rgba(0,0,0,0.08);color:var(--text-dim);">${o.delivery_status==='delivered'?'Доставлен':o.delivery_status==='shipped'?'В пути':o.delivery_status==='preparing'?'Готовится':'Ожидает'}</span>
              ${isAdmin ? `<span class="badge" style="background:${o.cost_paid?'rgba(0,185,86,0.12)':'rgba(232,162,58,0.12)'};color:${o.cost_paid?'var(--accent)':'var(--orange)'};">${o.cost_paid?'✅ Поставщик':'⏳ Поставщик'}</span>` : ''}
            </div>
          </div>
          <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;">
            <button class="btn-secondary" style="font-size:11px;padding:5px 8px;" onclick="openEditOrderAmountSheet(${o.id}, ${o.total_amount})">✏️ Сумма</button>
          </div>
        </div>
      `}).join('') || `<div class="empty-state"><div class="icon">📦</div><div class="title">Заказов нет</div></div>`}
    </div>
  `;
}

function setOrderFilter(type, value) {
  _ordersFilter[type] = value;
  if (type === 'delivery' && value !== 'all') { /* keep payment filter */ }
  refreshOrdersList();
}

function setOrderDays(days) {
  _ordersFilter.days = days;
  if (days > 0) { _ordersFilter.dateFrom = ''; _ordersFilter.dateTo = ''; }
  refreshOrdersList();
}

function toggleOrderDatePicker() {
  const el = document.getElementById('order-date-picker');
  if (el) el.style.display = el.style.display === 'none' ? 'flex' : 'none';
}

function applyOrderDateFilter() {
  _ordersFilter.days = 0;
  refreshOrdersList();
}

async function refreshOrdersList() {
  const c = document.getElementById('main-content');
  if (c && S.route === 'orders') c.innerHTML = await loadOrdersContent();
}

function openAssignManagerSheet(orderId, currentManagerId) {
  sb.from('profiles').select('id,full_name').eq('is_active', true).then(({ data: managers }) => {
    openSheet(`
      <div class="sheet-header"><h3>👤 Назначить менеджера</h3><button class="close-x" onclick="closeSheet()">✕</button></div>
      <div class="sheet-body">
        <div style="font-size:13px;color:var(--text-dim);margin-bottom:12px;">Выбери менеджера для этого заказа — его продажи учтутся в отчёте.</div>
        ${(managers||[]).map(m => `
          <div class="card" style="display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;${m.id===currentManagerId?'border-color:var(--accent);':''}" onclick="assignManagerToOrder(${orderId},'${m.id}')">
            <div style="flex:1;font-weight:600;">${escapeHtml(m.full_name||m.email)}</div>
            ${m.id===currentManagerId?'<span style="color:var(--accent);">✓</span>':''}
          </div>
        `).join('')}
        <button class="btn-secondary" style="margin-top:8px;" onclick="assignManagerToOrder(${orderId}, null)">❌ Убрать менеджера</button>
      </div>
    `);
  });
}

async function assignManagerToOrder(orderId, managerId) {
  await withLoading(async () => {
    const { error } = await sb.from('orders').update({ manager_id: managerId }).eq('id', orderId);
    if (error) throw error;
    closeSheet();
    showToast('Менеджер назначен ✓');
    refreshOrdersList();
  });
}
...
async function openOrderDetailSheet(orderId) {
  const { data: order, error } = await sb
    .from('orders')
    .select('*, clients(full_name,phone,address), deals(id,title,deal_items(id,qty,price,mattress_width,mattress_length,cost_price_at_sale,products(id,name,cost_per_cm,cost_price)))')
    .eq('id', orderId).single();
  if (error) { showToast('Не удалось загрузить заказ', 'error'); return; }
  S.cache.currentOrder = order;

  // Берём первый товар из сделки для автозаполнения
  const dealItems = order.deals?.deal_items || [];
  const firstItem = dealItems[0];
  const autoProduct = firstItem?.products;
  const autoWidth = firstItem?.mattress_width || order.mattress_width || 180;
  const autoLength = firstItem?.mattress_length || order.mattress_length || 200;
  const autoCostPerCm = autoProduct?.cost_per_cm || (autoProduct?.cost_price ? autoProduct.cost_price / 180 : 0);
  const autoCost = firstItem?.cost_price_at_sale || (autoCostPerCm ? Math.round(autoCostPerCm * autoWidth) : 0);
    ...
    <div class="sheet-body">
      ...
      ${dealItems.length ? `
        ...
        <div class="card" style="padding:10px 14px;">
          ${dealItems.map(it => {
            const w = it.mattress_width || 180;
            const l = it.mattress_length || 200;
            const cpa = it.cost_price_at_sale;
            return `
            <div style="padding:6px 0;border-bottom:1px solid var(--border);">
              <div style="font-weight:600;font-size:13.5px;">${escapeHtml(it.products?.name || '')} ${w}×${l} см</div>
              <div style="font-size:12px;color:var(--text-dim);">${it.qty} шт · ${fmtMoney(it.price)}${cpa ? ` · себест: ${fmtMoney(cpa)}` : ''}</div>
            </div>
          `}).join('')}
        </div>
      ` : ''}

      <div class="section-title">Оплата клиента</div>
      ...
      <div class="section-title">Себестоимость (оплата поставщику)</div>
      <div class="card" style="background:${order.cost_paid ? 'rgba(0,185,86,0.08)' : 'rgba(232,162,58,0.08)'};border-color:${order.cost_paid ? 'var(--accent)' : 'var(--orange)'};">
        <div style="font-size:12.5px;color:var(--text-dim);margin-bottom:8px;">
          ${order.cost_paid
            ? `✅ Оплачено поставщику: ${fmtMoney(order.cost_paid_amount || 0)} · ${order.cost_paid_at ? fmtDate(order.cost_paid_at) : ''}`
            : `⏳ Ещё не оплачено поставщику`}
        </div>
        ${autoProduct ? `
          <div style="background:var(--bg-elevated);border-radius:8px;padding:8px 10px;margin-bottom:10px;font-size:12.5px;">
            📦 Из сделки: <b>${escapeHtml(autoProduct.name)}</b> ${autoWidth}×${autoLength} см
            ${autoCost > 0 ? `· <b style="color:var(--accent);">${fmtMoney(autoCost)}</b>` : ''}
          </div>
        ` : ''}
        <div style="display:flex;gap:10px;">
          <div class="field" style="flex:1;"><label>Ширина (см)</label>
            <input type="number" id="order-mattress-width" value="${autoWidth}" oninput="recalcOrderCost(${orderId})">
          </div>
          <div class="field" style="flex:1;"><label>Длина (см)</label>
            <input type="number" id="order-mattress-length" value="${autoLength}" oninput="recalcOrderCost(${orderId})">
          </div>
        </div>
        <div class="field"><label>Модель матраса</label>
          <select id="order-mattress-model" onchange="recalcOrderCost(${orderId})">
            <option value="">— выбери модель —</option>
            ${(await sb.from('products').select('id,name,cost_per_cm,cost_price').eq('is_active', true)).data?.map(p =>
              `<option value="${p.id}"
                data-cpc="${p.cost_per_cm || (p.cost_price ? (p.cost_price/180).toFixed(4) : 0)}"
                ${(order.mattress_model == p.id || autoProduct?.id == p.id) ? 'selected' : ''}>${escapeHtml(p.name)}</option>`
            ).join('') || ''}
          </select>
        </div>
        <div id="order-cost-calc-result" style="font-size:12.5px;color:var(--text-dim);margin-bottom:8px;"></div>
        <div class="field"><label>Сумма себестоимости (сом) — можно изменить вручную</label>
          <input type="number" id="order-cost-amount" value="${order.cost_paid_amount || autoCost || ''}" placeholder="Авторасчёт или введи вручную">
        </div>
        <div class="field"><label>Примечание (необязательно)</label>
          <input type="text" id="order-cost-notes" value="${escapeHtml(order.cost_notes || '')}" placeholder="напр. оплата наличными">
        </div>
        <div class="btn-row">
          ${!order.cost_paid ? `<button class="btn-primary" onclick="markOrderCostPaid(${orderId})">✅ Отметить как оплачено</button>` : ''}
          ${order.cost_paid ? `<button class="btn-secondary" onclick="unmarkOrderCostPaid(${orderId})">Отменить отметку</button>` : ''}
        </div>
      </div>
  ...
  // Автоматически рассчитываем себестоимость при открытии если есть данные
  if (autoProduct?.id) setTimeout(() => recalcOrderCost(orderId), 100);
}

function recalcOrderCost(orderId) {
  const modelSelect = document.getElementById('order-mattress-model');
  const widthInput = document.getElementById('order-mattress-width');
  const costInput = document.getElementById('order-cost-amount');
  const resultDiv = document.getElementById('order-cost-calc-result');
  if (!modelSelect || !widthInput) return;

  const selectedOption = modelSelect.options[modelSelect.selectedIndex];
  const cpc = parseFloat(selectedOption?.dataset?.cpc) || 0;
  const width = parseInt(widthInput.value) || 180;

  if (cpc > 0 && width > 0) {
    const calc = Math.round(cpc * width);
    resultDiv.innerHTML = `Авторасчёт: ${cpc.toFixed(2)} сом/см × ${width} см = <b>${fmtMoney(calc)}</b>`;
    // Автозаполняем поле если оно пустое
    if (!costInput.value) costInput.value = calc;
  } else {
    resultDiv.innerHTML = `<span style="color:var(--orange);">Нет данных о себестоимости для этой модели — введи вручную</span>`;
  }
}

async function markOrderCostPaid(orderId) {
  const amount = parseFloat(document.getElementById('order-cost-amount').value) || 0;
  const notes = document.getElementById('order-cost-notes').value.trim();
  const width = parseInt(document.getElementById('order-mattress-width')?.value) || null;
  const modelId = document.getElementById('order-mattress-model')?.value || null;
  if (!amount) { showToast('Укажи сумму себестоимости', 'error'); return; }
  await withLoading(async () => {
    const { error } = await sb.from('orders').update({
      cost_paid: true,
      cost_paid_amount: amount,
      cost_paid_at: new Date().toISOString(),
      cost_notes: notes || null,
      mattress_width: width,
      mattress_model: modelId,
    }).eq('id', orderId);
    if (error) throw error;
    showToast('Оплата поставщику отмечена ✅');
    closeSheet();
    navigate('orders');
  });
}

async function unmarkOrderCostPaid(orderId) {
  if (!confirm('Отменить отметку оплаты поставщику?')) return;
  await withLoading(async () => {
    const { error } = await sb.from('orders').update({ cost_paid: false, cost_paid_amount: 0, cost_paid_at: null }).eq('id', orderId);
    if (error) throw error;
...
async function renderSettings() {
  ...
  return `
    <div class="page">
      ...
      ${S.profile.role === 'admin' ? `
        <button class="btn-secondary" style="margin-bottom:14px;" onclick="navigate('health')">🔍 Статус системы</button>
        <button class="btn-secondary" style="margin-bottom:14px;" onclick="navigate('ai_costs')">💸 Расходы AI</button>
        <button class="btn-secondary" style="margin-bottom:14px;" onclick="navigate('dialog_export')">📋 Анализ диалогов</button>
        <button class="btn-secondary" style="margin-bottom:14px;" onclick="navigate('managers')">👥 Менеджеры</button>
        <button class="btn-secondary" style="margin-bottom:14px;" onclick="navigate('templates')">📝 Шаблоны сообщений</button>
        ...
        <button class="btn-secondary" style="margin-bottom:14px;" onclick="downloadOrdersCsv()">📤 Экспорт заказов (CSV)</button>
        <button class="btn-secondary" style="margin-bottom:14px;" onclick="navigate('finance')">💰 Финансовый отчёт</button>
        <button class="btn-secondary" style="margin-bottom:14px;" onclick="navigate('conversation_analysis')">🔍 Анализ разговоров</button>
      ` : ''}
...
let _calcState = { width: 180, length: 200, discountPercent: 0 };
    ...
    <div class="page">
      <div class="calc-input-card">
        ...
        <div class="calc-size-row">
          ...
          <span class="x">×</span>
          <input type="number" id="calc-length" value="${_calcState.length}" placeholder="Длина" oninput="onCalcInputChange()">
        </div>
        <div style="font-size:11px;color:var(--text-faint);margin-top:6px;">Длина обычно 200 см — измени если нужен другой размер</div>
        ...
        <div style="margin-top:10px;display:flex;gap:8px;">
          <div style="flex:1;">
            <div style="font-size:11px;color:var(--text-faint);margin-bottom:4px;">Скидка %</div>
            <input type="number" id="calc-custom-discount" placeholder="напр. 12" value="${[0,10,15,20].includes(_calcState.discountPercent) ? '' : _calcState.discountPercent}" oninput="setCalcDiscountByPercent(parseFloat(this.value)||0)">
          </div>
          <div style="flex:1;">
            <div style="font-size:11px;color:var(--text-faint);margin-bottom:4px;">Скидка в сом (для 1-й модели)</div>
            <input type="number" id="calc-discount-amount" placeholder="напр. 300" oninput="setCalcDiscountByAmount(parseFloat(this.value)||0)">
          </div>
        </div>
      </div>
...
function onCalcInputChange() {
  ...
  _calcState.length = parseFloat(document.getElementById('calc-length').value) || 200;
  recalcCalculator();
}


function setCalcDiscountByPct(pct) {
  _calcState.discountPercent = pct;
  recalcCalculator();
  // Обновляем поле итоговой суммы
  const el = document.getElementById('calc-final-amount');
  if (!el) return;
  const w = _calcState.width || 180;
  const first = (S.cache.allProducts||[]).find(p=>p.price_per_cm);
  if (first) el.value = pct > 0 ? Math.round(first.price_per_cm * w * (1 - pct/100)) : '';
}

function setCalcDiscountByFinal(final) {
  const w = _calcState.width || 180;
  const first = (S.cache.allProducts||[]).find(p=>p.price_per_cm);
  if (!first || !final) return;
  const base = first.price_per_cm * w;
  const pct = Math.max(0, Math.round((1 - final/base)*100*10)/10);
  _calcState.discountPercent = pct;
  const el = document.getElementById('calc-custom-discount');
...
function setCalcDiscount(percent, fromCustom = false) {
  ...
  if (!fromCustom) {
    const el = document.getElementById('calc-custom-discount');
    if (el) el.value = [0,10,15,20].includes(percent) ? '' : percent;
  }
  const finalEl = document.getElementById('calc-final-amount');
  if (finalEl) {
    const w = _calcState.width || 180;
    const first = (S.cache.allProducts||[]).find(p=>p.price_per_cm);
    if (first && percent > 0) finalEl.value = Math.round(first.price_per_cm * w * (1 - percent/100));
    else finalEl.value = '';
  }
  recalcCalculator();
}
// old setCalcDiscount replaced above
function _old_setCalcDiscount(percent, fromCustom = false) {
  _calcState.discountPercent = percent;
  if (!fromCustom) {
    const customEl = document.getElementById('calc-custom-discount');
    if (customEl) customEl.value = percent > 0 && ![10,15,20].includes(percent) ? percent : '';
  }
  // Обновляем поле суммы скидки
  updateCalcDiscountAmount();
  recalcCalculator();
}

function setCalcDiscountByPercent(percent) {
  _calcState.discountPercent = percent;
  updateCalcDiscountAmount();
  recalcCalculator();
}

function setCalcDiscountByAmount(amount) {
  // Считаем % по первой модели с price_per_cm
  const width = _calcState.width || 180;
  const firstProduct = (S.cache.allProducts || []).find(p => p.price_per_cm);
  if (!firstProduct) return;
  const basePrice = firstProduct.price_per_cm * width;
  if (basePrice <= 0) return;
  const percent = Math.round(amount / basePrice * 100 * 10) / 10;
  _calcState.discountPercent = percent;
  const customEl = document.getElementById('calc-custom-discount');
  if (customEl) customEl.value = percent;
  recalcCalculator();
}

function updateCalcDiscountAmount() {
  const amountEl = document.getElementById('calc-discount-amount');
  if (!amountEl) return;
  const width = _calcState.width || 180;
  const firstProduct = (S.cache.allProducts || []).find(p => p.price_per_cm);
  if (!firstProduct || !_calcState.discountPercent) { amountEl.value = ''; return; }
  const basePrice = firstProduct.price_per_cm * width;
  const discountAmount = Math.round(basePrice * _calcState.discountPercent / 100);
  amountEl.value = discountAmount;
}
...
function renderCalcResults(products) {
  ...
  const length = _calcState.length || 200;
  ...
  const results = products.map(p => {
    ...
    const nonStandardLength = length !== 200;
    const costPerCm = p.cost_per_cm || (p.cost_price ? p.cost_price / 180 : 0);
    const costForWidth = costPerCm ? Math.round(costPerCm * width) : 0;
    const margin = Math.round(discounted - costForWidth);
    const marginPct = discounted > 0 && costForWidth > 0 ? Math.round(margin / discounted * 100) : null;
    return { product: p, rawPrice, discounted, belowMin, exceedsMax, maxAllowed, costForWidth, margin, marginPct, nonStandardLength };
  ...
  return results.map(r => `
    <div class="calc-result-card ${r === cheapest ? 'best' : ''}">
      <div>
        ...
        <div class="model-sub">${width} × ${length} см ${r.product.price_per_cm ? '· ' + r.product.price_per_cm + ' сом/см' : ''}</div>
        ${r.nonStandardLength ? `<div style="font-size:11px;color:var(--blue);margin-top:2px;">ℹ️ Нестандартная длина ${length} см</div>` : ''}
        ...
        ${r.costForWidth > 0 ? `
          <div style="margin-top:6px;font-size:11.5px;color:var(--text-dim);">
            ${S.profile?.role==='admin'?`Себест: <b>${fmtMoney(r.costForWidth)}</b>`:''}
            ${r.marginPct !== null ? ` · ${S.profile?.role==='admin'?`Прибыль: <b style='color:var(--accent);'>${fmtMoney(r.margin)} (${r.marginPct}%)</b>`:''}` : ''}
          </div>
        ` : ''}
      </div>
      <div style="text-align:right;">
        ...
        <button style="margin-top:6px;background:var(--violet-bg);border:1px solid var(--violet);color:var(--violet);border-radius:8px;padding:5px 10px;font-size:11px;font-weight:700;"
          onclick="attachCalcResultToDeal(${r.product.id},'${escapeHtml(r.product.name).replace(/'/g,"\\'")}',${r.discounted.toFixed(2)},${width},${length},${r.costForWidth})">В сделку →</button>
      </div>
    </div>
  `).join('');
}

function attachCalcResultToDeal(productId, productName, price, width, length = 200, costForWidth = 0) {
  openSheet(`
    <div class="sheet-header"><h3>Добавить в сделку</h3><button class="close-x" onclick="closeSheet()">✕</button></div>
    <div class="sheet-body">
      <div class="card">
        <div style="font-weight:600;">${escapeHtml(productName)} (${width}×${length})</div>
        <div style="color:var(--accent);font-weight:700;margin-top:4px;">${fmtMoney(price)}</div>
        ${costForWidth > 0 ? `<div style="font-size:12px;color:var(--text-dim);margin-top:2px;">Себестоимость: ${fmtMoney(costForWidth)} · Прибыль: ${fmtMoney(Math.round(price - costForWidth))}</div>` : ''}
      </div>
    ...
    <div class="sheet-footer">
      <button class="btn-primary" onclick="submitCalcDealAttach(${productId},'${escapeHtml(productName).replace(/'/g,"\\'")}',${price},${width},${length},${costForWidth})">Создать сделку с этим товаром</button>
    </div>
  `);
}
...
async function submitCalcDealAttach(productId, productName, price, width = 180, length = 200, costForWidth = 0) {
  if (!_calcDealClientId) { showToast('Выбери клиента из списка', 'error'); return; }
  await withLoading(async () => {
    const firstStage = S.stages[0];
    const sizeStr = `${width}×${length}`;
    const { data: deal, error } = await sb.from('deals').insert({
      client_id: _calcDealClientId,
      stage_id: firstStage.id,
      title: `${productName} ${sizeStr}`,
      amount: price,
      assigned_to: S.user.id,
    }).select().single();
    if (error) throw error;

    // Сохраняем позицию с размерами и себестоимостью
    await sb.from('deal_items').insert({
      deal_id: deal.id,
      product_id: productId,
      qty: 1,
      price,
      mattress_width: width,
      mattress_length: length,
      cost_price_at_sale: costForWidth || null,
    });

    await sb.from('interactions').insert({
      client_id: _calcDealClientId, deal_id: deal.id, type: 'system',
      content: `Создана сделка через калькулятор: ${productName} ${sizeStr} — ${fmtMoney(price)}${costForWidth > 0 ? ` (себест: ${fmtMoney(costForWidth)})` : ''}`,
      created_by: S.user.id,
    });
...
async function loadAnalyticsContent(days) {
  ...
  const [dealsRes, ordersRes, itemsRes, clientsRes, messagesRes] = await Promise.all([
    ...
    sb.from('clients').select('id,lead_status,conversation_owner,ai_managed,created_at').gte('created_at', periodStart.toISOString()),
    sb.from('messages').select('client_id,sender').gte('created_at', periodStart.toISOString()),
  ]);

  const deals = dealsRes.data || [];
  const orders = ordersRes.data || [];
  let items = itemsRes.data || [];
  items = items.filter(i => i.deals);
  const newClients = clientsRes.data || [];
  const messages = messagesRes.data || [];

  // Статистика по клиентам
  const clientsWithMessages = new Set(messages.map(m => m.client_id));
  const clientsWithAiReply = new Set(messages.filter(m => m.sender === 'ai_agent').map(m => m.client_id));
  const hotClients = newClients.filter(c => c.lead_status === 'hot').length;
  const warmClients = newClients.filter(c => c.lead_status === 'warm').length;
  const coldClients = newClients.filter(c => c.lead_status === 'cold').length; // только связанные с периодом
  ...
  return `
    <div class="page">
      ...
      ${topProducts.length ? `
        <div class="section-title">Воронка клиентов</div>
        <div class="card">
          ${[
            ['Новых клиентов', newClients.length, 'var(--blue)'],
            ['Написали (с перепиской)', clientsWithMessages.size, 'var(--violet)'],
            ['AI ответил', clientsWithAiReply.size, 'var(--accent)'],
            ['Создано сделок', deals.length, 'var(--gold)'],
            ['Купили', wonDeals.length, 'var(--accent)'],
          ].map(([label, count, color]) => `
            <div style="margin-bottom:10px;">
              <div style="display:flex;justify-content:space-between;font-size:12.5px;margin-bottom:4px;">
                <span>${label}</span><span style="font-weight:700;color:${color};">${count}</span>
              </div>
              <div style="height:6px;background:var(--bg-elevated);border-radius:6px;overflow:hidden;">
                <div style="height:100%;width:${newClients.length ? Math.round(count/newClients.length*100) : 0}%;background:${color};border-radius:6px;"></div>
              </div>
            </div>
          `).join('')}
          <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap;">
            <span style="background:rgba(255,100,100,0.15);color:var(--red);padding:3px 10px;border-radius:10px;font-size:11.5px;">🔴 Холодных: ${coldClients}</span>
            <span style="background:rgba(232,162,58,0.15);color:var(--orange);padding:3px 10px;border-radius:10px;font-size:11.5px;">🟡 Тёплых: ${warmClients}</span>
            <span style="background:rgba(0,185,86,0.15);color:var(--accent);padding:3px 10px;border-radius:10px;font-size:11.5px;">🟢 Горячих: ${hotClients}</span>
          </div>
        </div>
      ...
      ${isHumanOwned ? `
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 16px;background:rgba(232,162,58,0.12);border-bottom:1px solid rgba(232,162,58,0.3);flex-shrink:0;">
          ...
          <div style="display:flex;gap:6px;">
            <button style="background:var(--bg-card);border:1px solid var(--border-light);color:var(--text-dim);border-radius:8px;padding:5px 8px;font-size:11px;" onclick="loadAiSuggestions(${clientId})">💡 Варианты</button>
            <button style="background:var(--accent);color:#fff;border:none;border-radius:8px;padding:6px 12px;font-size:11.5px;font-weight:700;" onclick="returnAiToConversation(${clientId})">Вернуть AI</button>
          </div>
        </div>
      ` : `
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:8px 16px;background:rgba(0,185,86,0.08);border-bottom:1px solid rgba(0,185,86,0.2);flex-shrink:0;">
          <span style="font-size:11.5px;color:var(--accent);font-weight:600;">🤖 AI активен в этом диалоге</span>
          <div style="display:flex;gap:6px;">
            <button style="background:var(--bg-card);border:1px solid var(--border-light);color:var(--text-dim);border-radius:8px;padding:5px 8px;font-size:11px;" onclick="loadAiSuggestions(${clientId})">💡 Варианты</button>
            <button style="background:var(--bg-card);border:1px solid var(--border-light);color:var(--text-dim);border-radius:8px;padding:5px 10px;font-size:11px;font-weight:600;flex-shrink:0;" onclick="diagnoseAiSilence(${clientId})">Почему AI молчит?</button>
          </div>
        </div>
      `}
      <div id="ai-diagnostic-result"></div>
      <div class="chat-messages" id="chat-messages-container">
        <div class="loader-wrap"><div class="spinner"></div></div>
      </div>
      <div id="ai-suggestions-bar" style="display:none;padding:8px 12px;border-top:1px solid var(--border);background:var(--bg-elevated);flex-shrink:0;">
        <div style="font-size:11px;color:var(--text-faint);margin-bottom:6px;">💡 Варианты ответа от AI:</div>
        <div id="ai-suggestions-list" style="display:flex;flex-direction:column;gap:5px;"></div>
      </div>
      <div id="templates-bar" style="display:none;padding:8px 12px;border-top:1px solid var(--border);background:var(--bg-elevated);flex-shrink:0;max-height:280px;overflow-y:auto;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
          <span style="font-size:11px;color:var(--text-faint);">📝 Шаблоны:</span>
          <button onclick="hideTemplatesBar()" style="background:none;border:none;color:var(--text-faint);font-size:14px;">✕</button>
        </div>
        <input type="text" placeholder="Поиск..." oninput="filterChatTemplates(this.value)" style="margin-bottom:8px;font-size:12px;">
        <div style="display:flex;gap:4px;overflow-x:auto;padding-bottom:6px;scrollbar-width:none;margin-bottom:8px;">
          <button onclick="filterChatTemplatesCat('all')" class="tpl-cat-btn active" style="white-space:nowrap;padding:3px 8px;border-radius:10px;font-size:11px;border:1px solid var(--border);background:var(--violet);color:#fff;">Все</button>
          ${Object.entries(TEMPLATE_CATS).filter(([k])=>k!=='all').map(([k,v])=>`<button onclick="filterChatTemplatesCat('${k}')" class="tpl-cat-btn" style="white-space:nowrap;padding:3px 8px;border-radius:10px;font-size:11px;border:1px solid var(--border);background:var(--bg-card);color:var(--text-dim);">${v}</button>`).join('')}
        </div>
        <div id="chat-templates-list"></div>
      </div>
      <div class="chat-input-row">
        <button onclick="toggleTemplatesBar()" style="background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:8px 10px;font-size:16px;flex-shrink:0;">📝</button>
        <input type="text" id="chat-input" placeholder="Написать сообщение..." onkeydown="if(event.key==='Enter')sendChatMessage()" oninput="hideSuggestions()">
...
/**
 * Вызывает ai-agent напрямую для этого клиента и показывает реальный результат
 * (ответил/пропустил/ошибка) прямо в чате — без необходимости лезть в логи Supabase.
 */
async function diagnoseAiSilence(clientId) {
  const box = document.getElementById('ai-diagnostic-result');
  box.innerHTML = `<div style="padding:10px 16px;font-size:12px;color:var(--text-dim);">Проверяю...</div>`;
  try {
    const { data: sessionData } = await sb.auth.getSession();
    const token = sessionData.session?.access_token;
    ...
    if (!res.ok || data.success === false) {
      message = `⚠️ Ошибка: ${data.error || 'неизвестная ошибка'}`;
      color = 'var(--red)';
    } else if (data.skipped) {
      message = `⏸️ AI пропустил: ${data.skipped}`;
      color = 'var(--orange)';
    } else if (data.escalated) {
      message = `🙋 AI передал менеджеру: ${data.reason || 'без причины'}`;
      color = 'var(--orange)';
    } else if (data.replied) {
      message = `✅ AI ответил успешно — обновляю чат`;
      color = 'var(--accent)';
      await loadChatMessages(clientId);
    } else {
      message = `ℹ️ Ответ: ${JSON.stringify(data)}`;
    }

    box.innerHTML = `<div style="padding:10px 16px;font-size:12px;color:${color};border-bottom:1px solid var(--border);">${escapeHtml(message)}</div>`;
  } catch (err) {
    box.innerHTML = `<div style="padding:10px 16px;font-size:12px;color:var(--red);">Ошибка запроса: ${escapeHtml(err.message)}</div>`;
  }
}
...
async function loadActivityLogsContent(filter) {
  ...
  const filterBar = `
    ...
    <div style="display:flex;justify-content:flex-end;margin-bottom:10px;">
      <button class="btn-secondary" style="color:var(--red);font-size:12px;" onclick="clearActivityLogs('${filter}')">Очистить ${filter === 'all' ? 'все логи' : 'эту категорию'}</button>
    </div>
  `;
...
function generateCSV(headers, rows, filename) {
  ...
  const lines = [
    headers.join(SEP),
    ...rows.map(r => r.map(v => String(v === null || v === undefined ? '' : v).replace(/[\t\n\r]/g, ' ')).join(SEP))
  ];
...
async function downloadClientsCsv() {
  try {
    ...
    const headers = ['ID', 'Имя', 'Телефон', 'Instagram', 'Город', 'Адрес', 'Источник', 'Статус лида', 'Заметки', 'Дата создания'];
    ...
    // Используем ; как разделитель — Excel открывает корректно
    const csvContent = ['sep=;', headers.join(';'), ...rows.map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(';'))].join('\r\n');
  ...
  } catch (err) { showToast(err.message || 'Ошибка экспорта', 'error'); }
}
...
async function renderNotifications() {
  ...
  return `
    <div class="page">
      <div style="display:flex;justify-content:flex-end;margin-bottom:10px;">
        <button class="btn-secondary" style="color:var(--red);font-size:12px;" onclick="clearAllNotifications()">Очистить все</button>
      </div>
      ${notifications.map(n => {
        const cfg = typeConfig[n.type] || { icon: '🔔', label: '' };
        return `
        <div class="card" style="display:flex;align-items:flex-start;gap:10px;">
          <div style="flex:1;" ${n.client_id ? `onclick="navigate('client_detail',{id:${n.client_id}})"` : ''}>
          ...
          <button onclick="deleteNotification(${n.id})" style="background:none;border:none;color:var(--text-faint);font-size:16px;padding:4px;flex-shrink:0;">✕</button>
        </div>
      `;
      }).join('')}
    </div>
...
async function clearActivityLogs(category) {
  const msg = category === 'all' ? 'Удалить все логи?' : `Удалить логи категории "${category}"?`;
  if (!confirm(msg)) return;
  let query = sb.from('activity_logs').delete();
...
// ============================================
// ГОТОВЫЕ ОТВЕТЫ AI ДЛЯ МЕНЕДЖЕРА
// ============================================

async function loadAiSuggestions(clientId) {
  const bar = document.getElementById('ai-suggestions-bar');
  const list = document.getElementById('ai-suggestions-list');
  if (!bar || !list) return;

  try {
    const { data: history } = await sb.from('messages').select('*').eq('client_id', clientId)
    ...
    if (messages[messages.length-1]?.role === 'assistant') {
      messages.push({ role: 'user', content: '(менеджер отвечает)' });
    }

    const { data: sessionData } = await sb.auth.getSession();
    const token = sessionData.session?.access_token;

    const aiRes = await fetch(`${SUPABASE_URL}/functions/v1/ai-suggestions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`, 'apikey': SUPABASE_ANON_KEY },
      body: JSON.stringify({ client_id: clientId, history: messages.slice(-8) })
    });
    ...
    list.innerHTML = data.suggestions.map(s => `
      <button onclick="useSuggestion(this.dataset.text)"
        data-text="${escapeHtml(s)}"
        style="text-align:left;background:var(--bg-card);border:1px solid var(--border-light);border-radius:8px;padding:7px 10px;font-size:12.5px;color:var(--text);line-height:1.4;">
        ${escapeHtml(s)}
      </button>
    `).join('');
    ...
    console.error('Ошибка генерации вариантов ответа:', err);
  }
}
...
// ============================================
// ЭКСПОРТ ЗАКАЗОВ (пункт 11)
// ============================================

async function downloadOrdersCsv() {
  try {
    const { data: orders, error } = await sb.from('orders')
      .select('*, clients(full_name,phone), deals(title,assigned_to,profiles:assigned_to(full_name),deal_items(qty,price,mattress_width,mattress_length,products(name)))')
      .order('created_at', { ascending: false });
    if (error) throw error;
    const headers = ['Номер заказа','Дата','Клиент','Телефон','Менеджер','Товар','Размер','Сумма','Оплачено','Остаток','Статус оплаты','Статус доставки','Адрес'];
    const rows = (orders||[]).map(o => {
      const item = o.deals?.deal_items?.[0];
      const product = item?.products?.name || o.deals?.title || '';
      const size = item?.mattress_width ? `${item.mattress_width}x${item.mattress_length||200}` : '';
      return [
        o.order_number||o.id,
        new Date(o.created_at).toLocaleDateString('ru'),
        o.clients?.full_name||'', o.clients?.phone||'',
        o.deals?.profiles?.full_name||'',
        product, size,
        Number(o.total_amount||0), Number(o.paid_amount||0),
        Number(o.total_amount||0)-Number(o.paid_amount||0),
        o.payment_status==='paid'?'Оплачен':o.payment_status==='partial'?'Частично':'Не оплачен',
        o.delivery_status==='delivered'?'Доставлен':o.delivery_status==='shipped'?'В пути':o.delivery_status==='preparing'?'Готовится':'Ожидает',
        o.delivery_address||'',
      ];
    });
    generateCSV(headers, rows, `orders_${new Date().toISOString().slice(0,10)}.tsv`);
  } catch(err) { showToast(err.message||'Ошибка', 'error'); }
}

// ============================================
// ФИНАНСОВЫЙ ОТЧЁТ (пункт 20)
// ============================================

let _financePeriod = 30;

async function renderFinanceReport() {
  return await loadFinanceContent(_financePeriod);
}

async function loadFinanceContent(days) {
  _financePeriod = days;
  let periodStart, periodEnd;
  if (days === 0 && _financeCustomFrom && _financeCustomTo) {
    periodStart = new Date(_financeCustomFrom);
    periodEnd = new Date(_financeCustomTo);
  ...
  const { data: orders, error } = await sb
    .from('orders')
    .select('*, clients(full_name,phone), deals(id,title,deal_items(qty,price,mattress_width,mattress_length,cost_price_at_sale,products(name,cost_price,cost_per_cm)))')
  ...
  let totalRevenue = 0, totalReceived = 0, totalCost = 0, totalCostPaid = 0;
  let unpaidOrders = 0, costUnpaidOrders = 0;

  const rows = (orders || []).map(o => {
    const revenue = Number(o.total_amount) || 0;
    const received = Number(o.paid_amount) || 0;

    // Приоритет себестоимости:
    // 1. cost_paid_amount из заказа (если вручную отмечено оплаченным)
    // 2. cost_price_at_sale из позиций сделки (заполняется через калькулятор/форму)
    // 3. cost_per_cm × ширина из позиций
    // 4. cost_price × qty из товаров (базовая закупочная)
    const costFromField = Number(o.cost_paid_amount) || 0;
    const costItems = o.deals?.deal_items || [];
    const costFromItems = costItems.reduce((s, i) => {
      if (i.cost_price_at_sale) return s + Number(i.cost_price_at_sale) * i.qty;
      const cpc = i.products?.cost_per_cm || (i.products?.cost_price ? i.products.cost_price / 180 : 0);
      if (cpc && i.mattress_width) return s + Math.round(cpc * i.mattress_width) * i.qty;
      return s + (i.qty * Number(i.products?.cost_price || 0));
    }, 0);
    const cost = costFromField > 0 ? costFromField : costFromItems;

    // Информация о товаре для отображения
    const firstItem = costItems[0];
    const productInfo = firstItem?.products?.name
      ? `${firstItem.products.name}${firstItem.mattress_width ? ` ${firstItem.mattress_width}×${firstItem.mattress_length || 200}` : ''}`
      : null;

    const costPaid = o.cost_paid ? cost : 0;
    const profit = revenue - cost;

    totalRevenue += revenue;
    totalReceived += received;
    totalCost += cost;
    totalCostPaid += costPaid;
    if (o.payment_status !== 'paid') unpaidOrders++;
    if (!o.cost_paid && cost > 0) costUnpaidOrders++;

    return { ...o, revenue, received, cost, costPaid, profit, productInfo };
  });

  const totalReceivable = totalRevenue - totalReceived;
  const totalCostUnpaid = totalCost - totalCostPaid;
  const totalProfit = totalRevenue - totalCost;

  return `
    <div class="page">
      <div class="tabs">
        <button class="tab ${days===7?'active':''}" onclick="switchFinancePeriod(7)">7 дней</button>
        <button class="tab ${days===30?'active':''}" onclick="switchFinancePeriod(30)">30 дней</button>
        <button class="tab ${days===90?'active':''}" onclick="switchFinancePeriod(90)">90 дней</button>
        <button class="tab ${days===0?'active':''}" onclick="showCustomPeriodFinance()">📅 Свой</button>
      </div>
      ${days===0 ? `
        <div style="display:flex;gap:8px;margin-bottom:12px;align-items:center;">
          <input type="date" id="fin-date-from" style="flex:1;" value="${_financeCustomFrom||''}">
          <span style="color:var(--text-dim);">—</span>
          <input type="date" id="fin-date-to" style="flex:1;" value="${_financeCustomTo||''}">
          <button class="btn-primary" style="padding:8px 12px;white-space:nowrap;" onclick="applyCustomPeriodFinance()">OK</button>
        </div>
      ` : ''}
      </div>

      <div class="stat-grid">
        <div class="stat-card"><div class="label">💰 Выручка</div><div class="value gold">${fmtMoney(totalRevenue)}</div></div>
        <div class="stat-card"><div class="label">✅ Получено</div><div class="value">${fmtMoney(totalReceived)}</div></div>
        <div class="stat-card"><div class="label">⏳ Дебиторка</div><div class="value" style="color:var(--orange)">${fmtMoney(totalReceivable)}</div><div class="delta">${unpaidOrders} заказов не оплачено</div></div>
        ${isAdmin ? `
        <div class="stat-card"><div class="label">📦 Себестоимость</div><div class="value">${fmtMoney(totalCost)}</div></div>
        <div class="stat-card"><div class="label">💸 Оплачено поставщику</div><div class="value">${fmtMoney(totalCostPaid)}</div></div>
        <div class="stat-card"><div class="label">🔴 Долг поставщику</div><div class="value" style="color:var(--red)">${fmtMoney(totalCostUnpaid)}</div><div class="delta">${costUnpaidOrders} заказов</div></div>
        <div class="stat-card" style="grid-column:1/-1"><div class="label">📈 Валовая прибыль</div><div class="value" style="font-size:24px;color:var(--accent)">${fmtMoney(totalProfit)}</div><div class="delta">маржа ${totalRevenue > 0 ? Math.round(totalProfit/totalRevenue*100) : 0}%</div></div>
        ` : ''}
      </div>

      ${isAdmin ? `<button class="btn-secondary" style="margin-bottom:12px;" onclick="exportFinanceCSV()">📤 Экспорт в CSV</button>` : ''}

      <div class="section-title">Заказы (${rows.length})</div>
      ${rows.length ? rows.map(o => `
        <div class="card" style="font-size:13px;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
            <div>
              <div style="font-weight:700;">${escapeHtml(o.clients?.full_name || '—')}</div>
              <div style="color:var(--text-dim);font-size:11.5px;">${o.order_number || 'без номера'} · ${fmtDate(o.created_at)}</div>
              ${o.productInfo ? `<div style="color:var(--text-faint);font-size:11.5px;">📦 ${escapeHtml(o.productInfo)} см</div>` : ''}
            </div>
            <div style="text-align:right;">
              <div style="font-weight:700;color:var(--gold)">${fmtMoney(o.revenue)}</div>
              <div style="font-size:11px;color:${o.payment_status==='paid'?'var(--accent)':'var(--orange)'};">${o.payment_status==='paid'?'✅ Клиент оплатил':'⏳ Клиент не оплатил'}</div>
            </div>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;color:var(--text-dim);font-size:11.5px;flex-wrap:wrap;gap:4px;">
            ${isAdmin ? `<span>Себест: <b>${fmtMoney(o.cost)}</b></span>
            <span>Прибыль: <b style="color:var(--accent);">${fmtMoney(o.profit)}</b></span>
            <span style="color:${o.cost_paid?'var(--accent)':'var(--orange)'};">
              ${o.cost_paid ? `✅ Поставщику оплачено` : `⏳ Поставщику не оплачено`}
            </span>` : ''}
          </div>
          <div style="display:flex;gap:6px;margin-top:6px;flex-wrap:wrap;">
            ${isAdmin && !o.cost_paid && o.cost > 0 ? `
              <button class="btn-secondary" style="font-size:11.5px;padding:5px 10px;" onclick="openOrderDetailSheet(${o.id})">Отметить оплату поставщику →</button>
            ` : ''}
            <button class="btn-secondary" style="font-size:11.5px;padding:5px 10px;" onclick="openEditOrderAmountSheet(${o.id}, ${o.revenue})">✏️ Изменить сумму</button>
          </div>
        </div>
      `).join('') : `<div style="text-align:center;color:var(--text-faint);padding:20px;">Заказов нет за этот период</div>`}
    </div>
  `;
}

let _financeCustomFrom = '';
let _financeCustomTo = '';

function showCustomPeriodFinance() {
  _financePeriod = 0;
  const c = document.getElementById('main-content');
  if (c) loadFinanceContent(0).then(h => c.innerHTML = h);
}

function applyCustomPeriodFinance() {
  _financeCustomFrom = document.getElementById('fin-date-from')?.value || '';
  _financeCustomTo = document.getElementById('fin-date-to')?.value || '';
  if (!_financeCustomFrom || !_financeCustomTo) { showToast('Выбери обе даты', 'error'); return; }
  loadFinanceContent(0).then(h => document.getElementById('main-content').innerHTML = h);
}

async function switchFinancePeriod(days) {
  const c = document.getElementById('main-content');
  c.innerHTML = `<div class="loader-wrap"><div class="spinner"></div></div>`;
  c.innerHTML = await loadFinanceContent(days);
}

async function markCostPaid(orderId, cost) {
  await withLoading(async () => {
    const { error } = await sb.from('orders').update({
      cost_paid: true,
      cost_paid_amount: cost,
      cost_paid_at: new Date().toISOString(),
    }).eq('id', orderId);
    if (error) throw error;
    showToast('Оплата поставщику отмечена');
...
// ============================================
// АНАЛИЗ РАЗГОВОРОВ (пункт 21)
// ============================================

async function renderConversationAnalysis() {
  return `
    <div class="page">
      <div class="card" style="text-align:center;padding:24px 16px;">
        <div style="font-size:32px;margin-bottom:12px;">🔍</div>
        <div style="font-weight:700;font-size:16px;margin-bottom:8px;">Анализ переписок</div>
        <div style="font-size:13px;color:var(--text-dim);margin-bottom:20px;">AI проанализирует последние разговоры с клиентами и выдаст выводы: что работает, где теряются клиенты, какие возражения чаще всего встречаются.</div>
        <div class="tabs" style="margin-bottom:16px;">
          <button class="tab active" id="ana-tab-7" onclick="setAnalysisPeriod(7,this)">7 дней</button>
          <button class="tab" id="ana-tab-30" onclick="setAnalysisPeriod(30,this)">30 дней</button>
        </div>
        <button class="btn-primary" onclick="runConversationAnalysis()" id="ana-btn">Запустить анализ</button>
        <div style="font-size:11px;color:var(--text-faint);margin-top:8px;">Анализ занимает 30-60 секунд</div>
      </div>
...
function setAnalysisPeriod(days, btn) {
  _analysisDays = days;
  document.querySelectorAll('#ana-tab-7,#ana-tab-30').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
}

async function runConversationAnalysis() {
  const btn = document.getElementById('ana-btn');
  const result = document.getElementById('ana-result');
  if (!btn || !result) return;

  btn.textContent = 'Анализирую...';
  btn.disabled = true;
  result.innerHTML = `<div class="loader-wrap"><div class="spinner"></div></div>`;

  try {
    const periodStart = new Date();
    periodStart.setDate(periodStart.getDate() - _analysisDays);

    // Берём переписки за период
    ...
    if (!messages?.length) {
      result.innerHTML = `<div class="card" style="text-align:center;color:var(--text-faint);">Нет сообщений за этот период</div>`;
      btn.textContent = 'Запустить анализ';
      btn.disabled = false;
      return;
    }

    // Группируем по клиентам и составляем краткое резюме каждого разговора
    const byClient = {};
    messages.forEach(m => {
      if (!byClient[m.client_id]) byClient[m.client_id] = { name: m.clients?.full_name || 'клиент', msgs: [] };
      byClient[m.client_id].msgs.push(`${m.direction==='inbound'?'Клиент':'AI/Менеджер'}: ${(m.content||'').slice(0,200)}`);
    });

    const conversations = Object.values(byClient).slice(0, 30); // максимум 30 разговоров за раз
    const conversationSummary = conversations.map(c => `[${c.name}]: ${c.msgs.join(' | ')}`).join('\n\n');
    ...
    if (data.analysis) {
      result.innerHTML = `
        <div class="card">
          <div style="font-weight:700;font-size:14px;margin-bottom:12px;">📊 Результаты анализа ${conversations.length} разговоров за ${_analysisDays} дней</div>
          <div style="font-size:13px;line-height:1.7;white-space:pre-wrap;">${escapeHtml(data.analysis)}</div>
        </div>
      `;
    } else {
      result.innerHTML = `<div class="card" style="color:var(--red);">Ошибка анализа: ${escapeHtml(data.error || 'неизвестная ошибка')}</div>`;
    }
  } catch (err) {
    result.innerHTML = `<div class="card" style="color:var(--red);">Ошибка: ${escapeHtml(err.message)}</div>`;
  } finally {
    btn.textContent = 'Запустить анализ';
    btn.disabled = false;
  }
...
const TEMPLATE_CATS = {
  all: 'Все', greeting: '👋 Приветствие', price: '💰 Цена', size: '📏 Размер',
  firmness: '🛏️ Жёсткость', back_pain: '🤕 Боли', for_whom: '👨‍👩‍👧 Для кого',
  recommendation: '🛌 Подбор', advantages: '⭐ Плюсы модели', price_promo: '💵 Акция',
  promo: '🎁 Срочность', delivery: '🚚 Доставка', warranty: '🛡️ Гарантия',
  order: '✅ Заказ', followup: '⏳ Follow-up', obj_price: '❌ Дорого',
  obj_think: '🤔 Подумаю', obj_later: '⏰ Позже', obj_compare: '🔍 Сравниваю',
  obj_budget: '💸 Бюджет', obj_advice: '🤝 Совет', obj_quality: '🤨 Качество',
  escalation: '🤖 Менеджер', upsell: '🛒 Допродажи', ads: '📲 Реклама',
  closing: '🙏 Завершение', faq: '❓ FAQ', general: '📁 Общие',
  get_phone: '📞 Телефон', get_address: '🏠 Адрес', address: '📍 Адрес магазина',
  stock: '📦 Наличие', custom_size: '📐 Нестандарт', production: '🏭 Производство',
  care: '🧹 Уход', return_client: '❤️ Возврат', bought: '✅ Уже купил',
};

async function renderTemplates() {
  const { data: templates } = await sb.from('message_templates').select('*').eq('is_active', true).order('sort_order');
  S.cache.allTemplates = templates || [];
  S.cache.templateTexts = {};
  (templates || []).forEach(t => { S.cache.templateTexts[t.id] = t.content; });
  return buildTemplatesHtml();
}

function buildTemplatesHtml() {
  const templates = S.cache.allTemplates || [];
  ...
  const grouped = {};
  filtered.forEach(t => {
    const label = TEMPLATE_CATS[t.category] || t.category;
    if (!grouped[label]) grouped[label] = [];
    grouped[label].push(t);
  });

  const filterBar = Object.entries(TEMPLATE_CATS).map(([k,v]) => `
    <button onclick="setTemplateFilter('${k}')"
      style="display:inline-block;margin-right:5px;padding:4px 10px;border-radius:14px;font-size:11px;font-weight:600;white-space:nowrap;border:1px solid var(--border);background:${_templateFilter===k?'var(--violet)':'var(--bg-card)'};color:${_templateFilter===k?'#fff':'var(--text-dim)'};">
      ${v}
    </button>`).join('');

  const listHtml = buildTemplatesListHtml(filtered, grouped);

  return `
    <div class="page">
      <button class="btn-primary" style="margin-bottom:10px;" onclick="openCreateTemplateSheet()">+ Новый шаблон</button>
      <input type="text" id="tpl-search-input" placeholder="🔍 Поиск..." value="${escapeHtml(_templateSearch)}"
        oninput="_templateSearch=this.value; refreshTemplateListOnly()"
        style="margin-bottom:10px;">
      <div id="tpl-filter-bar" style="overflow-x:auto;white-space:nowrap;padding-bottom:8px;margin-bottom:12px;scrollbar-width:none;">
        ${filterBar}
      </div>
      <div id="tpl-list">${listHtml}</div>
    </div>`;
}

function buildTemplatesListHtml(filtered, grouped) {
  if (!filtered) {
    const templates = S.cache.allTemplates || [];
  ...
  if (filtered.length === 0) return `<div class="empty-state"><div class="icon">📝</div><div class="title">Ничего не найдено</div></div>`;
  ...
  return Object.entries(grouped).map(([cat, items]) => `
    ...
    ${items.map(t => `
      <div class="card" style="padding:10px 12px;">
        ...
        <div style="font-size:12px;color:var(--text-dim);line-height:1.5;margin-bottom:8px;white-space:pre-wrap;">${escapeHtml(t.content.slice(0,120))}${t.content.length>120?'…':''}</div>
        <div style="display:flex;gap:6px;">
          <button class="btn-secondary" style="font-size:11px;flex:1;" onclick="copyTemplateById(${t.id})">📋 Копировать</button>
          <button class="btn-secondary" style="font-size:11px;" onclick="editTemplate(${t.id})">✏️</button>
          <button class="btn-secondary" style="font-size:11px;color:var(--red);" onclick="deleteTemplate(${t.id})">🗑️</button>
        </div>
      </div>`).join('')}`).join('');
}

// Обновляет только список — НЕ трогает поле поиска (решает баг с клавиатурой)
function refreshTemplateListOnly() {
  const list = document.getElementById('tpl-list');
  if (list) list.innerHTML = buildTemplatesListHtml();
  ...
  if (bar) {
    bar.querySelectorAll('button').forEach(btn => {
      ...
      if (k) {
        btn.style.background = _templateFilter === k ? 'var(--violet)' : 'var(--bg-card)';
        btn.style.color = _templateFilter === k ? '#fff' : 'var(--text-dim)';
      }
    });
  }
}
...
function setTemplateFilter(cat) {
  _templateFilter = cat;
  refreshTemplateListOnly();
}

function refreshTemplatesList() {
  if (S.route === 'templates') {
    const el = document.getElementById('main-content');
    if (el) el.innerHTML = buildTemplatesHtml();
  }
}

async function copyTemplateById(id) {
  const text = S.cache.templateTexts?.[id];
  if (!text) { showToast('Текст не найден', 'error'); return; }
  try {
    await navigator.clipboard.writeText(text);
    showToast('Скопировано ✓');
  } catch {
    // Fallback для старых браузеров и мобильных
    ...
    try {
      ...
      showToast('Не удалось скопировать — скопируй вручную', 'error');
    }
...
function openCreateTemplateSheet() {
  const cats = { general: 'Общие', price: 'Цены', delivery: 'Доставка', objection: 'Возражения', followup: 'Follow-up' };
  openSheet(`
    <div class="sheet-header"><h3>Новый шаблон</h3><button class="close-x" onclick="closeSheet()">✕</button></div>
    <div class="sheet-body">
      <div class="field"><label>Название</label><input type="text" id="tpl-title" placeholder="напр. Прайс для Худжанда"></div>
      <div class="field"><label>Текст сообщения</label><textarea id="tpl-content" rows="4" placeholder="Текст который будет скопирован одним нажатием"></textarea></div>
    ...
    <div class="sheet-footer"><button class="btn-primary" onclick="submitCreateTemplate()">Сохранить</button></div>
  `);
}
...
async function submitCreateTemplate() {
  ...
  if (!title || !content) { showToast('Заполни название и текст', 'error'); return; }
  await withLoading(async () => {
    const { error } = await sb.from('message_templates').insert({ title, content, category: document.getElementById('tpl-cat').value });
    if (error) throw error;
    closeSheet(); showToast('Шаблон создан'); navigate('templates');
  });
}

async function editTemplate(id) {
  const { data: t } = await sb.from('message_templates').select('*').eq('id', id).single();
  if (!t) return;
  const cats = { general: 'Общие', price: 'Цены', delivery: 'Доставка', objection: 'Возражения', followup: 'Follow-up' };
  openSheet(`
    <div class="sheet-header"><h3>Редактировать</h3><button class="close-x" onclick="closeSheet()">✕</button></div>
    <div class="sheet-body">
      <div class="field"><label>Название</label><input type="text" id="tpl-title" value="${escapeHtml(t.title)}"></div>
      <div class="field"><label>Текст</label><textarea id="tpl-content" rows="4">${escapeHtml(t.content)}</textarea></div>
      <div class="field"><label>Категория</label>
        <select id="tpl-cat">
          ${Object.entries(cats).map(([v,l]) => `<option value="${v}" ${t.category===v?'selected':''}>${l}</option>`).join('')}
        </select>
      </div>
    </div>
    <div class="sheet-footer"><button class="btn-primary" onclick="submitEditTemplate(${id})">Сохранить</button></div>
  `);
}

async function submitEditTemplate(id) {
  await withLoading(async () => {
    const { error } = await sb.from('message_templates').update({
    ...
    if (error) throw error;
    closeSheet(); showToast('Сохранено'); navigate('templates');
  });
}
...
async function deleteTemplate(id) {
  ...
  showToast('Удалено'); navigate('templates');
}

// ============================================
// ИСТОРИЯ ЗАКАЗОВ В КАРТОЧКЕ КЛИЕНТА (пункт 11)
// ============================================

async function renderClientOrderHistory(clientId) {
  const { data: orders } = await sb.from('orders').select('*, deals(title)').eq('client_id', clientId).order('created_at', { ascending: false });
  if (!orders?.length) return `<div style="color:var(--text-faint);font-size:12.5px;text-align:center;padding:10px;">Заказов пока нет</div>`;
  const total = orders.reduce((s, o) => s + Number(o.total_amount || 0), 0);
  return `
    <div style="font-size:12px;color:var(--text-faint);margin-bottom:8px;">Всего куплено: <b style="color:var(--gold)">${fmtMoney(total)}</b></div>
    ${orders.map(o => `
      <div style="padding:8px 0;border-bottom:1px solid var(--border);font-size:12.5px;">
        <div style="display:flex;justify-content:space-between;">
          ...
          <span style="color:var(--gold);font-weight:600;">${fmtMoney(o.total_amount)}</span>
        </div>
        <div style="color:var(--text-faint);margin-top:2px;">${fmtDate(o.created_at)} · ${o.payment_status === 'paid' ? '✅ Оплачен' : '⏳ Не оплачен'}</div>
      </div>
    `).join('')}
  `;
}
...
// ============================================
// КЭШИРОВАНИЕ AI-SUGGESTIONS (пункт 16)
// ============================================

const _suggestionsCache = new Map(); // clientId -> { time, suggestions }
const SUGGESTIONS_CACHE_TTL = 5 * 60 * 1000; // 5 минут

async function loadAiSuggestionsWithCache(clientId) {
  const cached = _suggestionsCache.get(clientId);
  if (cached && Date.now() - cached.time < SUGGESTIONS_CACHE_TTL) {
    // Отображаем из кэша без нового запроса к Claude
    const bar = document.getElementById('ai-suggestions-bar');
    const list = document.getElementById('ai-suggestions-list');
    if (bar && list) {
      list.innerHTML = cached.suggestions.map(s => `
        <button onclick="useSuggestion(this.dataset.text)" data-text="${escapeHtml(s)}"
          style="text-align:left;background:var(--bg-card);border:1px solid var(--border-light);border-radius:8px;padding:7px 10px;font-size:12.5px;color:var(--text);line-height:1.4;">
          ${escapeHtml(s)}
        </button>
      `).join('');
  ...
  // Нет кэша — запрашиваем и сохраняем
  await loadAiSuggestions(clientId);
}

// Переопределяем оригинальную функцию чтобы она кэшировала результат
const _originalLoadAiSuggestions = loadAiSuggestions;
// Сохраняем результат в кэш после успешного получения — патчим в конце загрузки чата

// ============================================
// HEALTH-CHECK ДАШБОРД (пункт 17)
// ============================================

async function renderHealthCheck() {
  const checks = [
    { name: 'AI-агент', key: 'ai_agent_health' },
    { name: 'Instagram webhook', key: 'instagram_health' },
    { name: 'Бэкап', key: 'backup_health' },
  ];

  // Проверяем через activity_logs — последние успешные действия
  const { data: logs } = await sb
    .from('activity_logs')
    .select('action,created_at')
    .in('action', ['ai_replied', 'backup_created', 'ai_escalated', 'lead_received'])
    .order('created_at', { ascending: false })
    .limit(20);

  const now = Date.now();
  const lastAiReply = logs?.find(l => l.action === 'ai_replied');
  const lastBackup = logs?.find(l => l.action === 'backup_created');

  const hoursAgo = (dateStr) => dateStr ? Math.round((now - new Date(dateStr).getTime()) / 3600000) : null;

  const aiHours = hoursAgo(lastAiReply?.created_at);
  const backupHours = hoursAgo(lastBackup?.created_at);

  const statusColor = (ok) => ok ? 'var(--accent)' : 'var(--red)';
  const statusText = (ok, detail) => ok ? `✅ ${detail}` : `⚠️ ${detail}`;
  ...
  return `
    <div class="page">
      ...
      <div class="card">
        <div style="font-weight:600;margin-bottom:12px;">AI-агент</div>
        ${aiHours !== null
          ? `<div style="color:${statusColor(aiHours < 24)};">${statusText(aiHours < 24, `Последний ответ ${aiHours}ч назад`)}</div>`
          : `<div style="color:var(--orange);">⚠️ Нет данных о работе AI</div>`}
      </div>

      <div class="card">
        <div style="font-weight:600;margin-bottom:12px;">Ежедневный бэкап</div>
        ${backupHours !== null
          ? `<div style="color:${statusColor(backupHours < 25)};">${statusText(backupHours < 25, `Последний бэкап ${backupHours}ч назад`)}</div>`
          : `<div style="color:var(--orange);">⚠️ Бэкап ещё не запускался</div>`}
        <button class="btn-secondary" style="margin-top:10px;font-size:12px;" onclick="downloadFullBackup()">💾 Скачать бэкап сейчас</button>
      </div>

      <div class="card">
        <div style="font-weight:600;margin-bottom:12px;">Cron задачи</div>
        <div style="font-size:12.5px;color:var(--text-dim);line-height:1.8;">
          ✅ ai-reengagement — каждый час<br>
          ✅ ai-followup-check — ежедневно в 6:00<br>
          ✅ ai-task-executor — каждые 15 мин<br>
          ✅ backup-export-daily — ежедневно в 2:00
        </div>
      </div>

      <div class="card">
        <div style="font-weight:600;margin-bottom:12px;">Последние ошибки</div>
        ${await renderLastErrors()}
      </div>

      <div class="card">
        <div style="font-weight:600;margin-bottom:10px;">Токен Instagram</div>
        <div style="font-size:12.5px;color:var(--orange);">⚠️ Токены Instagram истекают примерно раз в 2 месяца. Перегенерируй их в Meta for Developers → Instagram → Access Tokens.</div>
      </div>
    </div>
  `;
}
...
async function renderLastErrors() {
  ...
  if (!errors?.length) return `<div style="color:var(--accent);font-size:12.5px;">✅ Ошибок нет</div>`;
  return errors.map(e => `
    <div style="font-size:12px;color:var(--red);padding:4px 0;border-bottom:1px solid var(--border);">
      <div>${escapeHtml(e.description?.slice(0, 80) || '')}…</div>
      <div style="color:var(--text-faint);font-size:11px;">${timeAgo(e.created_at)}</div>
    </div>
  `).join('');
}

// ============================================
// ШАБЛОНЫ В ЧАТЕ С КЛИЕНТОМ
// ============================================

let _chatTemplatesCat = 'all';
let _chatTemplatesSearch = '';

async function toggleTemplatesBar() {
  const bar = document.getElementById('templates-bar');
  if (!bar) return;
  if (bar.style.display === 'none') {
    bar.style.display = 'block';
    hideSuggestions();
    // Загружаем если ещё нет
    if (!S.cache.allTemplates) {
      const { data } = await sb.from('message_templates').select('*').eq('is_active', true).order('sort_order');
      S.cache.allTemplates = data || [];
      S.cache.templateTexts = {};
      (data || []).forEach(t => { S.cache.templateTexts[t.id] = t.content; });
    }
    renderChatTemplates();
  } else {
...
function hideTemplatesBar() {
  const bar = document.getElementById('templates-bar');
  if (bar) bar.style.display = 'none';
}

function filterChatTemplates(q) {
  _chatTemplatesSearch = q;
  renderChatTemplates();
}

function filterChatTemplatesCat(cat) {
  _chatTemplatesCat = cat;
  // Обновляем стили кнопок
  document.querySelectorAll('.tpl-cat-btn').forEach(b => {
    b.style.background = 'var(--bg-card)';
    b.style.color = 'var(--text-dim)';
  });
  event.target.style.background = 'var(--violet)';
...
function renderChatTemplates() {
  ...
  if (!list) return;
  const all = S.cache.allTemplates || [];
  const q = _chatTemplatesSearch.toLowerCase();
  const filtered = all.filter(t => {
    const matchCat = _chatTemplatesCat === 'all' || t.category === _chatTemplatesCat;
    const matchQ = !q || t.title.toLowerCase().includes(q) || t.content.toLowerCase().includes(q);
    return matchCat && matchQ;
  });

  if (!filtered.length) {
    list.innerHTML = '<div style="text-align:center;color:var(--text-faint);font-size:12px;padding:10px;">Ничего не найдено</div>';
    return;
  }

  list.innerHTML = filtered.map(t => `
    <div style="border:1px solid var(--border);border-radius:8px;padding:8px 10px;margin-bottom:6px;cursor:pointer;"
      onclick="useChatTemplate(${t.id})">
      <div style="font-weight:600;font-size:12px;margin-bottom:3px;">${escapeHtml(t.title)}</div>
      <div style="font-size:11.5px;color:var(--text-dim);line-height:1.4;">${escapeHtml(t.content.slice(0,100))}${t.content.length>100?'…':''}</div>
    </div>
  `).join('');
}
...
// ============================================
// РАСХОДЫ AI — СТАТИСТИКА ТОКЕНОВ
// ============================================

let _aiCostsPeriod = 7;

async function renderAiCosts() {
  // Загружаем сохранённый начальный баланс
  const savedBalance = parseFloat(localStorage.getItem('ai_initial_balance') || '0');
  return await loadAiCostsContent(7, savedBalance);
}

async function loadAiCostsContent(days, initialBalance) {
  _aiCostsPeriod = days;
  if (initialBalance === undefined) initialBalance = parseFloat(localStorage.getItem('ai_initial_balance') || '0');

  let periodStart, periodEnd;
  if (days === 0 && _aiCostsCustomFrom && _aiCostsCustomTo) {
    periodStart = new Date(_aiCostsCustomFrom);
    periodEnd = new Date(_aiCostsCustomTo);
    periodEnd.setHours(23,59,59);
  } else {
    periodStart = new Date();
    periodStart.setDate(periodStart.getDate() - (days || 7));
    periodEnd = new Date();
  }

  // Для расчёта остатка берём ВСЕ расходы за всё время
  const [periodRes, allTimeRes] = await Promise.all([
    sb.from('ai_usage_stats').select('*, clients(full_name)').gte('created_at', periodStart.toISOString()).order('created_at', { ascending: false }),
    sb.from('ai_usage_stats').select('cost_usd'),
  ]);

  if (periodRes.error) {
    return `<div class="page"><div class="card" style="color:var(--orange);">Таблица ai_usage_stats ещё не создана. Выполни SQL: setup_ai_usage_stats.sql</div></div>`;
  }
  ...
  const allTimeCost = allRows.reduce((s, r) => s + Number(r.cost_usd || 0), 0);
  const avgCost = rows.length ? totalCost / rows.length : 0;
  const estimatedBalance = initialBalance > 0 ? Math.max(0, initialBalance - allTimeCost) : null;

  // Группируем по клиентам
  const byClient = {};
  rows.forEach(r => {
    const name = r.clients?.full_name || 'Неизвестный';
    if (!byClient[name]) byClient[name] = { count: 0, cost: 0, tokens: 0 };
    byClient[name].count++;
    byClient[name].cost += Number(r.cost_usd || 0);
    byClient[name].tokens += r.total_tokens || 0;
  });
  ...
  return `
    <div class="page">
      ...
      <div class="card" style="margin-bottom:14px;">
        <div style="font-weight:600;font-size:13.5px;margin-bottom:10px;">💳 Баланс Anthropic</div>
        <div style="display:flex;gap:10px;margin-bottom:10px;">
          <div class="field" style="flex:1;margin:0;">
            <label style="font-size:11px;">Начальный баланс ($)</label>
            <input type="number" id="initial-balance-input" step="0.01" value="${initialBalance || ''}"
              placeholder="напр. 20.00"
              style="font-size:13px;">
          </div>
          <button class="btn-primary" style="align-self:flex-end;padding:8px 14px;font-size:13px;"
            onclick="saveInitialBalance()">Сохранить</button>
        </div>
        ${estimatedBalance !== null ? `
          <div style="display:flex;justify-content:space-between;padding:10px;background:var(--bg-elevated);border-radius:10px;">
            <div>
              <div style="font-size:11px;color:var(--text-faint);">Начальный баланс</div>
              <div style="font-weight:700;">$${initialBalance.toFixed(2)}</div>
            </div>
            <div>
              <div style="font-size:11px;color:var(--text-faint);">Потрачено всего</div>
              <div style="font-weight:700;color:var(--orange);">$${allTimeCost.toFixed(4)}</div>
            </div>
            <div>
              <div style="font-size:11px;color:var(--text-faint);">Остаток (расчётный)</div>
              <div style="font-weight:700;color:${estimatedBalance < 2 ? 'var(--red)' : 'var(--accent)'};">
                $${estimatedBalance.toFixed(4)}
                ${estimatedBalance < 2 ? ' ⚠️' : ''}
              </div>
            </div>
          </div>
          ${estimatedBalance < 2 ? `<div style="color:var(--red);font-size:12px;margin-top:8px;">⚠️ Баланс заканчивается — пополни на console.anthropic.com</div>` : ''}
        ` : `<div style="font-size:12px;color:var(--text-faint);">Введи начальный баланс чтобы видеть остаток</div>`}
      </div>

      <div class="tabs">
        <button class="tab ${days===7?'active':''}" onclick="switchAiCostsPeriod(7)">7 дней</button>
        <button class="tab ${days===30?'active':''}" onclick="switchAiCostsPeriod(30)">30 дней</button>
        <button class="tab ${days===90?'active':''}" onclick="switchAiCostsPeriod(90)">90 дней</button>
        <button class="tab ${days===0?'active':''}" onclick="showCustomPeriodAiCosts()">📅 Свой</button>
      </div>
      ${days===0 ? `
        <div style="display:flex;gap:8px;margin-bottom:12px;align-items:center;">
          <input type="date" id="ai-date-from" style="flex:1;" value="${_aiCostsCustomFrom||''}">
          <span style="color:var(--text-dim);">—</span>
          <input type="date" id="ai-date-to" style="flex:1;" value="${_aiCostsCustomTo||''}">
          <button class="btn-primary" style="padding:8px 12px;" onclick="applyCustomPeriodAiCosts()">OK</button>
        </div>
      ` : ''}

      <div class="stat-grid">
        <div class="stat-card"><div class="label">💸 Расходов за период</div><div class="value" style="color:var(--orange);">$${totalCost.toFixed(4)}</div></div>
        <div class="stat-card"><div class="label">📨 Ответов AI</div><div class="value">${rows.length}</div></div>
        <div class="stat-card"><div class="label">💰 Средний за ответ</div><div class="value">$${avgCost.toFixed(4)}</div></div>
        <div class="stat-card"><div class="label">📥 Input токенов</div><div class="value" style="font-size:16px;">${totalInput.toLocaleString()}</div></div>
        <div class="stat-card"><div class="label">📤 Output токенов</div><div class="value" style="font-size:16px;">${totalOutput.toLocaleString()}</div></div>
        <div class="stat-card"><div class="label">📊 Всего токенов</div><div class="value" style="font-size:16px;">${(totalInput+totalOutput).toLocaleString()}</div></div>
      </div>

      ${topClients.length ? `
        <div class="section-title">Топ клиентов по расходам</div>
        ${topClients.map(([name, d]) => `
          <div class="card" style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;">
            <div>
              ...
              <div style="font-size:11.5px;color:var(--text-dim);">${d.count} ответов · ${d.tokens.toLocaleString()} токенов</div>
            </div>
            <div style="font-weight:700;color:var(--orange);">$${d.cost.toFixed(4)}</div>
          </div>
        `).join('')}
      ` : ''}

      <div class="section-title">Последние ответы</div>
      ${rows.slice(0, 20).map(r => `
        <div class="card" style="padding:8px 14px;font-size:12.5px;">
          <div style="display:flex;justify-content:space-between;">
            <span style="font-weight:600;">${escapeHtml(r.clients?.full_name || '—')}</span>
            <span style="color:var(--orange);font-weight:700;">$${Number(r.cost_usd).toFixed(4)}</span>
          </div>
          <div style="color:var(--text-dim);margin-top:2px;">
            ${r.input_tokens} in · ${r.output_tokens} out · ${fmtDate(r.created_at)}
          </div>
        </div>
      `).join('') || `<div style="text-align:center;color:var(--text-faint);padding:20px;">Нет данных — статистика начнёт собираться после обновления ai-agent</div>`}
    </div>
  `;
}
...
function saveInitialBalance() {
  ...
  localStorage.setItem('ai_initial_balance', val.toString());
  showToast('Баланс сохранён');
  switchAiCostsPeriod(_aiCostsPeriod);
}

async function switchAiCostsPeriod(days) {
  const c = document.getElementById('main-content');
  c.innerHTML = `<div class="loader-wrap"><div class="spinner"></div></div>`;
  const savedBalance = parseFloat(localStorage.getItem('ai_initial_balance') || '0');
  c.innerHTML = await loadAiCostsContent(days, savedBalance);
}
...
async function renderDialogExport() {
  return `
    <div class="page">
      <div class="card">
        <div style="font-size:13.5px;font-weight:600;margin-bottom:8px;">📋 Экспорт диалогов для анализа</div>
        <div style="font-size:12.5px;color:var(--text-dim);margin-bottom:14px;">
          Выгружает переписки в читаемом формате — скопируй и отправь Claude для анализа почему клиенты не покупают.
        </div>
        ...
        <div class="field">
          <label>Показать клиентов</label>
          <select id="de-filter">
            <option value="all">Все кто писал</option>
            <option value="no_order" selected>Только без покупки</option>
            <option value="with_order">Только купившие</option>
          </select>
        </div>

        <div class="field">
          <label>Максимум клиентов</label>
          <select id="de-limit">
            <option value="20" selected>20 клиентов</option>
            <option value="50">50 клиентов</option>
          </select>
        </div>

        <button class="btn-primary" onclick="runDialogExport()" id="de-btn">📥 Выгрузить диалоги</button>
        <div id="de-status" style="font-size:12px;color:var(--accent);margin-top:8px;"></div>
      </div>

      <div id="de-stats" style="display:none;" class="card">
        <div style="font-weight:600;margin-bottom:10px;">📊 Статистика за период</div>
        <div id="de-stats-content"></div>
      </div>

      <div id="de-result-card" style="display:none;" class="card">
        <div style="font-weight:600;margin-bottom:6px;">📝 Текст для Claude</div>
        <div style="font-size:12px;color:var(--text-dim);margin-bottom:10px;">Нажми кнопку — скопируй — вставь в чат с Claude</div>
        <textarea id="de-result" readonly style="width:100%;height:300px;font-size:11px;font-family:monospace;background:var(--bg-elevated);border:1px solid var(--border);border-radius:8px;padding:10px;color:var(--text);resize:none;box-sizing:border-box;"></textarea>
        <button class="btn-primary" style="margin-top:8px;" onclick="copyDialogExport()">📋 Копировать всё</button>
      </div>
    </div>
  `;
}
...
async function runDialogExport() {
  const days = parseInt(document.getElementById('de-days').value);
  const filter = document.getElementById('de-filter').value;
  const limit = parseInt(document.getElementById('de-limit').value);
  const btn = document.getElementById('de-btn');
  const status = document.getElementById('de-status');

  btn.disabled = true;
  status.textContent = '🔄 Загружаю сообщения...';

  try {
    const periodStart = new Date();
    periodStart.setDate(periodStart.getDate() - days);

    // Загружаем все входящие сообщения за период
    const { data: inbound } = await sb.from('messages')
      .select('client_id')
      .gte('created_at', periodStart.toISOString())
      .eq('direction', 'inbound');

    const allClientIds = [...new Set((inbound || []).map(m => m.client_id))];
    status.textContent = `🔄 Найдено ${allClientIds.length} клиентов, загружаю данные...`;

    // Загружаем клиентов
    const { data: clients } = await sb.from('clients')
      .select('id,full_name,lead_status,city,phone')
      .in('id', allClientIds);

    // Загружаем заказы
    const { data: orders } = await sb.from('orders')
      .select('client_id,total_amount,payment_status,created_at')
    ...
    if (filter === 'no_order') filteredClients = filteredClients.filter(c => !ordersByClient[c.id]?.length);
    if (filter === 'with_order') filteredClients = filteredClients.filter(c => ordersByClient[c.id]?.length > 0);
    filteredClients = filteredClients.slice(0, limit);

    const filteredIds = filteredClients.map(c => c.id);

    // Загружаем все сообщения отфильтрованных клиентов
    ...
    const withOrders = allClientIds.filter(id => ordersByClient[id]?.length).length;
    const convRate = totalClients > 0 ? Math.round(withOrders / totalClients * 100) : 0;
    const silentAfterPrice = (clients || []).filter(c => {
      const msgs = msgByClient[c.id] || [];
      const lastMsg = msgs[msgs.length - 1];
      const hasPrice = msgs.some(m => m.direction === 'outbound' &&
        (m.content?.includes('сом') || m.content?.includes('нарх')));
      return hasPrice && lastMsg?.direction === 'outbound';
    }).length;

    document.getElementById('de-stats').style.display = 'block';
    document.getElementById('de-stats-content').innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:8px;">
        <div style="flex:1;min-width:100px;background:var(--bg-elevated);border-radius:8px;padding:10px;text-align:center;">
          <div style="font-size:20px;font-weight:700;">${totalClients}</div>
          <div style="font-size:11px;color:var(--text-dim);">Написали</div>
        </div>
        <div style="flex:1;min-width:100px;background:var(--bg-elevated);border-radius:8px;padding:10px;text-align:center;">
          <div style="font-size:20px;font-weight:700;color:var(--accent);">${withOrders}</div>
          <div style="font-size:11px;color:var(--text-dim);">Купили (${convRate}%)</div>
        </div>
        <div style="flex:1;min-width:100px;background:var(--bg-elevated);border-radius:8px;padding:10px;text-align:center;">
          <div style="font-size:20px;font-weight:700;color:var(--orange);">${silentAfterPrice}</div>
          <div style="font-size:11px;color:var(--text-dim);">Молчат после цены</div>
        </div>
      </div>
    `;

    // Формируем текст
    let out = `ДИАЛОГИ ORTHOSLEEP — АНАЛИЗ ПРОДАЖ\n`;
    out += `Период: ${days} дней | Всего: ${totalClients} | Купили: ${withOrders} (${convRate}%) | Молчат после цены: ${silentAfterPrice}\n`;
    out += `${'═'.repeat(60)}\n\n`;
    out += `ПРОСЬБА К CLAUDE: Проанализируй диалоги ниже и ответь:\n`;
    out += `1. Почему клиенты не покупают — конкретные причины\n`;
    out += `2. Какие ошибки делает AI в диалогах\n`;
    out += `3. Что конкретно написать в промпте чтобы исправить\n\n`;
    out += `${'═'.repeat(60)}\n\n`;

    filteredClients.forEach((client, idx) => {
      const msgs = msgByClient[client.id] || [];
      const clientOrders = ordersByClient[client.id] || [];
      const hasPurchase = clientOrders.length > 0;

      out += `[${ idx + 1 }] ${client.full_name || 'Без имени'}`;
      out += ` | ${hasPurchase ? '✅ КУПИЛ' : '❌ НЕ КУПИЛ'}`;
      if (client.city) out += ` | ${client.city}`;
      out += `\n`;

      if (hasPurchase) {
        const total = clientOrders.reduce((s, o) => s + Number(o.total_amount || 0), 0);
        out += `💰 Сумма заказа: ${total} сом\n`;
      }
      ...
      msgs.forEach(m => {
        ...
        const dateStr = dt.toLocaleDateString('ru', { day: '2-digit', month: '2-digit' });
        const timeStr = dt.toLocaleTimeString('ru', { hour: '2-digit', minute: '2-digit' });
        const who = m.direction === 'inbound' ? '👤 Клиент' : '🤖 AI';
        const text = (m.content || '').trim();
        out += `${dateStr} ${timeStr} ${who}:\n${text}\n\n`;
      });

      out += `${'═'.repeat(60)}\n\n`;
    });

    document.getElementById('de-result-card').style.display = 'block';
    document.getElementById('de-result').value = out;
    status.textContent = `✅ Готово! ${filteredClients.length} диалогов. Скопируй и отправь Claude.`;
...
async function copyDialogExport() {
  const text = document.getElementById('de-result').value;
  try {
    await navigator.clipboard.writeText(text);
    showToast('Скопировано ✓');
  } catch {
    document.getElementById('de-result').select();
    document.execCommand('copy');
    showToast('Скопировано ✓');
  }
}
...
// ============================================
// МЕНЕДЖЕРЫ — ПРОДАЖИ И БОНУСЫ
// ============================================

let _managersPeriod = 30;

async function renderManagers() {
  return await loadManagersContent(30);
}

async function loadManagersContent(days) {
  _managersPeriod = days;
  let periodStart, periodEnd, periodStartStr, periodEndStr;
  if (days === 0 && _managersCustomFrom && _managersCustomTo) {
    periodStart = new Date(_managersCustomFrom);
    periodEnd = new Date(_managersCustomTo);
    periodEnd.setHours(23,59,59);
  } else {
    periodStart = new Date();
    periodStart.setDate(periodStart.getDate() - (days || 30));
    periodEnd = new Date();
  }
  ...
  const { data: managers } = await sb.from('profiles').select('*').eq('is_active', true).order('full_name');

  // Загружаем продажи из CRM (orders через deals)
  const { data: crmOrders } = await sb.from('orders')
    .select('*, deals(assigned_to, amount), clients(full_name)')
    .gte('created_at', periodStart.toISOString())
    .eq('payment_status', 'paid');

  // Загружаем ручные продажи
  const { data: manualSales } = await sb.from('manual_sales')
    .select('*, profiles(full_name, bonus_percent)')
    .gte('sale_date', periodStartStr)
    .order('sale_date', { ascending: false });

  // Считаем статистику по каждому менеджеру
  const stats = (managers || []).map(m => {
    // CRM продажи — берём менеджера из deals.assigned_to (единая логика)
    const crmSales = (crmOrders || []).filter(o => o.deals?.assigned_to === m.id);
    const crmRevenue = crmSales.reduce((s, o) => s + Number(o.total_amount || 0), 0);

    // Ручные продажи
    const manual = (manualSales || []).filter(s => s.manager_id === m.id);
    const manualRevenue = manual.reduce((s, o) => s + Number(o.amount || 0), 0);
    const manualProfit = manual.reduce((s, o) => s + Number(o.profit || 0), 0);

    const totalRevenue = crmRevenue + manualRevenue;
    const totalSales = crmSales.length + manual.length;
    const bonusPct = Number(m.bonus_percent || 5);
    const bonus = Math.round(totalRevenue * bonusPct / 100);
    const avgCheck = totalSales > 0 ? Math.round(totalRevenue / totalSales) : 0;

    return { manager: m, crmSales: crmSales.length, manualSales: manual.length,
      totalSales, crmRevenue, manualRevenue, totalRevenue, manualProfit, bonus, bonusPct, avgCheck };
  });

  const totalRevenue = stats.reduce((s, m) => s + m.totalRevenue, 0);
  const totalSales = stats.reduce((s, m) => s + m.totalSales, 0);
  const totalBonus = stats.reduce((s, m) => s + m.bonus, 0);

  return `
    <div class="page">
      <button class="btn-primary" style="margin-bottom:12px;" onclick="openAddManualSaleSheet()">+ Добавить продажу</button>

      <div class="tabs" style="margin-bottom:14px;">
        <button class="tab ${days===7?'active':''}" onclick="reloadManagers(7)">7 дней</button>
        <button class="tab ${days===30?'active':''}" onclick="reloadManagers(30)">30 дней</button>
        <button class="tab ${days===90?'active':''}" onclick="reloadManagers(90)">90 дней</button>
        <button class="tab ${days===365?'active':''}" onclick="reloadManagers(365)">Год</button>
        <button class="tab ${days===0?'active':''}" onclick="showCustomPeriodManagers()">📅 Свой</button>
      </div>
      ${days===0 ? `
        <div style="display:flex;gap:8px;margin-bottom:12px;align-items:center;">
          <input type="date" id="mgr-date-from" style="flex:1;" value="${_managersCustomFrom||''}">
          <span style="color:var(--text-dim);">—</span>
          <input type="date" id="mgr-date-to" style="flex:1;" value="${_managersCustomTo||''}">
          <button class="btn-primary" style="padding:8px 12px;" onclick="applyCustomPeriodManagers()">OK</button>
        </div>
      ` : ''}

      <div class="stat-grid" style="margin-bottom:14px;">
        <div class="stat-card"><div class="label">💰 Общая выручка</div><div class="value">${fmtMoney(totalRevenue)}</div></div>
        <div class="stat-card"><div class="label">📦 Всего продаж</div><div class="value">${totalSales}</div></div>
        <div class="stat-card"><div class="label">🎁 Бонусы итого</div><div class="value" style="color:var(--orange);">${fmtMoney(totalBonus)}</div></div>
      </div>

      <div class="section-title">Менеджеры</div>
      ${stats.map(s => `
        <div class="card" onclick="navigate('manager_detail', {id:'${s.manager.id}', days:${days}})" style="cursor:pointer;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
            <div>
              <div style="font-weight:700;font-size:14px;">${escapeHtml(s.manager.full_name || s.manager.email)}</div>
              <div style="font-size:12px;color:var(--text-dim);">Бонус: ${s.bonusPct}% · Средний чек: ${fmtMoney(s.avgCheck)}</div>
            </div>
            <div style="text-align:right;">
              <div style="font-weight:700;color:var(--accent);font-size:15px;">${fmtMoney(s.totalRevenue)}</div>
              <div style="font-size:11.5px;color:var(--orange);">Бонус: ${fmtMoney(s.bonus)}</div>
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <span class="badge" style="background:var(--bg-elevated);">📦 ${s.totalSales} продаж</span>
            ${s.crmSales > 0 ? `<span class="badge" style="background:rgba(99,102,241,0.12);color:var(--violet);">CRM: ${s.crmSales}</span>` : ''}
            ${s.manualSales > 0 ? `<span class="badge" style="background:rgba(16,185,129,0.12);color:var(--accent);">Ручные: ${s.manualSales}</span>` : ''}
          </div>
        </div>
      `).join('') || `<div class="empty-state"><div class="icon">👥</div><div class="title">Нет менеджеров</div></div>`}

      <div class="section-title">Настройка бонусов</div>
      ${(managers || []).map(m => `
        <div class="card" style="display:flex;align-items:center;gap:10px;padding:10px 14px;">
          <div style="flex:1;font-weight:600;font-size:13px;">${escapeHtml(m.full_name || m.email)}</div>
          <input type="number" value="${m.bonus_percent || 5}" min="0" max="100" step="0.5"
            style="width:70px;text-align:center;font-size:13px;"
            onchange="updateManagerBonus('${m.id}', this.value)">
...
async function reloadManagers(days) {
  const c = document.getElementById('main-content');
  c.innerHTML = `<div class="loader-wrap"><div class="spinner"></div></div>`;
  c.innerHTML = await loadManagersContent(days);
}

async function updateManagerBonus(managerId, percent) {
  const { error } = await sb.from('profiles').update({ bonus_percent: parseFloat(percent) }).eq('id', managerId);
  if (error) { showToast('Ошибка сохранения', 'error'); return; }
  showToast('Бонус обновлён ✓');
}

async function renderManagerDetail(managerId) {
  const days = S.routeParams?.days || 30;
  const periodStart = new Date();
  periodStart.setDate(periodStart.getDate() - days);
  const periodStartStr = periodStart.toISOString().split('T')[0];

  const { data: manager } = await sb.from('profiles').select('*').eq('id', managerId).single();
  const { data: manualSales } = await sb.from('manual_sales')
    .select('*').eq('manager_id', managerId)
    .gte('sale_date', periodStartStr)
    .order('sale_date', { ascending: false });
  ...
  const crmSales = (crmOrders || []).filter(o => o.deals?.assigned_to === managerId);
  const manual = manualSales || [];

  const totalRevenue = crmSales.reduce((s,o) => s + Number(o.total_amount||0), 0)
    + manual.reduce((s,o) => s + Number(o.amount||0), 0);
  const totalSales = crmSales.length + manual.length;
  const bonusPct = Number(manager?.bonus_percent || 5);
  const bonus = Math.round(totalRevenue * bonusPct / 100);

  // Группируем по дням
  const byDay = {};
  [...crmSales.map(o => ({ date: o.created_at?.split('T')[0], amount: Number(o.total_amount||0), type: 'CRM', name: o.clients?.full_name || '—' })),
   ...manual.map(o => ({ date: o.sale_date, amount: Number(o.amount||0), type: 'Ручная', name: o.client_name }))
  ].forEach(s => {
  ...
  return `
    <div class="page">
      <div class="card" style="margin-bottom:14px;">
        <div style="font-weight:700;font-size:15px;margin-bottom:4px;">${escapeHtml(manager?.full_name || manager?.email || '—')}</div>
        <div style="font-size:12px;color:var(--text-dim);">Бонус: ${bonusPct}% от выручки</div>
      </div>

      <div class="stat-grid" style="margin-bottom:14px;">
        <div class="stat-card"><div class="label">💰 Выручка</div><div class="value">${fmtMoney(totalRevenue)}</div></div>
        <div class="stat-card"><div class="label">📦 Продаж</div><div class="value">${totalSales}</div></div>
        <div class="stat-card"><div class="label">🎁 Бонус</div><div class="value" style="color:var(--orange);">${fmtMoney(bonus)}</div></div>
        <div class="stat-card"><div class="label">💵 Средний чек</div><div class="value">${fmtMoney(totalSales > 0 ? Math.round(totalRevenue/totalSales) : 0)}</div></div>
      </div>

      <button class="btn-primary" style="margin-bottom:14px;" onclick="openAddManualSaleSheet('${managerId}')">+ Добавить продажу</button>

      <div class="section-title">По дням</div>
      ${Object.entries(byDay).sort((a,b) => b[0].localeCompare(a[0])).map(([date, sales]) => {
        const dayTotal = sales.reduce((s,o) => s + o.amount, 0);
        return `
          <div class="card" style="padding:10px 14px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
              <div style="font-weight:600;font-size:13px;">${new Date(date).toLocaleDateString('ru', {day:'numeric',month:'long'})}</div>
              <div style="font-weight:700;color:var(--accent);">${fmtMoney(dayTotal)}</div>
            </div>
            ${sales.map(s => `
              <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text-dim);padding:3px 0;border-top:1px solid var(--border);">
                <span>${escapeHtml(s.name)} <span style="color:var(--violet);font-size:11px;">${s.type}</span></span>
                <span>${fmtMoney(s.amount)}</span>
              </div>
            `).join('')}
          </div>
        `;
      }).join('') || `<div style="text-align:center;color:var(--text-faint);padding:20px;">Нет продаж за период</div>`}
    </div>
  `;
}

function openAddManualSaleSheet(managerId) {
  // Если не передан ID — берём текущего пользователя
  const mgrId = managerId || S.user?.id;
  openSheet(`
    <div class="sheet-header"><h3>+ Добавить продажу</h3><button class="close-x" onclick="closeSheet()">✕</button></div>
    <div class="sheet-body">
      <div class="field"><label>Имя клиента</label><input type="text" id="ms-client-name" placeholder="напр. Азиз Гуломзода"></div>
      <div class="field"><label>Телефон клиента</label><input type="tel" id="ms-client-phone" placeholder="+992..."></div>
      <div class="field"><label>Товар</label><input type="text" id="ms-product" placeholder="напр. Sultan 180×200"></div>
      <div class="field"><label>Сумма продажи (сом)</label><input type="number" id="ms-amount" placeholder="0"></div>
      <div class="field"><label>Себестоимость (сом)</label><input type="number" id="ms-cost" placeholder="0"></div>
      <div class="field"><label>Дата продажи</label><input type="date" id="ms-date" value="${new Date().toISOString().split('T')[0]}"></div>
      <div class="field"><label>Примечание</label><input type="text" id="ms-note" placeholder="необязательно"></div>
    </div>
    <div class="sheet-footer">
      <button class="btn-primary" onclick="submitManualSale('${mgrId}')">Сохранить продажу</button>
    </div>
  `);
}

async function submitManualSale(managerId) {
  const clientName = document.getElementById('ms-client-name').value.trim();
  const amount = parseFloat(document.getElementById('ms-amount').value) || 0;
  if (!clientName) { showToast('Укажи имя клиента', 'error'); return; }
  if (!amount) { showToast('Укажи сумму', 'error'); return; }

  const { data: manager } = await sb.from('profiles').select('bonus_percent').eq('id', managerId).single();
  const bonusPct = Number(manager?.bonus_percent || 5);
  ...
  await withLoading(async () => {
    const { error } = await sb.from('manual_sales').insert({
      ...
      client_phone: document.getElementById('ms-client-phone').value.trim() || null,
      product_name: document.getElementById('ms-product').value.trim() || null,
      amount,
      cost_price: cost,
      bonus,
      note: document.getElementById('ms-note').value.trim() || null,
      sale_date: document.getElementById('ms-date').value,
    });
    if (error) throw error;
    closeSheet();
    showToast(`Продажа добавлена! Бонус: ${fmtMoney(bonus)}`);
    navigate('managers');
  });
}

// ============================================
// ЭКСПОРТ ФИНАНСОВ В CSV
// ============================================

async function exportFinanceCSV() {
  const days = _financePeriod;
  let periodStart, periodEnd;
  if (days === 0 && _financeCustomFrom && _financeCustomTo) {
    periodStart = new Date(_financeCustomFrom);
    periodEnd = new Date(_financeCustomTo); periodEnd.setHours(23,59,59);
  } else {
    periodStart = new Date(); periodStart.setDate(periodStart.getDate() - (days||30));
    periodEnd = new Date();
  }
  const { data: orders } = await sb.from('orders')
    .select('*, clients(full_name,phone), deals(assigned_to,profiles:assigned_to(full_name),deal_items(qty,price,mattress_width,mattress_length,cost_price_at_sale,products(name,cost_price,cost_per_cm)))')
    .gte('created_at', periodStart.toISOString())
    .lte('created_at', periodEnd.toISOString())
    .order('created_at', { ascending: false });

  const headers = ['Номер заказа','Дата','Клиент','Телефон','Менеджер','Товар','Размер','Сумма','Оплачено','Статус оплаты','Себестоимость','Прибыль','Статус поставщика'];
  const rows = (orders||[]).map(o => {
    const item = o.deals?.deal_items?.[0];
    const product = item?.products?.name || '';
    const size = item?.mattress_width ? `${item.mattress_width}x${item.mattress_length||200}` : '';
    const cost = Number(o.cost_paid_amount||0);
    const revenue = Number(o.total_amount||0);
    return [
      o.order_number||'', new Date(o.created_at).toLocaleDateString('ru'),
      o.clients?.full_name||'', o.clients?.phone||'',
      o.deals?.profiles?.full_name||'',
      product, size, revenue, Number(o.paid_amount||0),
      o.payment_status==='paid'?'Оплачен':'Не оплачен',
      cost, revenue-cost,
      o.cost_paid?'Оплачено поставщику':'Не оплачено',
    ];
  });
  generateCSV(headers, rows, `finance_${periodStart.toISOString().slice(0,10)}.tsv`);
}

// ============================================
// РЕДАКТИРОВАНИЕ СУММЫ ЗАКАЗА
// ============================================

function openEditOrderAmountSheet(orderId, currentAmount) {
  openSheet(`
    <div class="sheet-header"><h3>✏️ Изменить сумму заказа</h3><button class="close-x" onclick="closeSheet()">✕</button></div>
    <div class="sheet-body">
      <div style="font-size:13px;color:var(--text-dim);margin-bottom:12px;">
        Текущая сумма: <b>${fmtMoney(currentAmount)}</b><br>
        Используй если сделали скидку клиенту после оформления заказа.
      </div>
      <div class="field">
        <label>Новая сумма (сом)</label>
        <input type="number" id="edit-order-amount" value="${currentAmount}" placeholder="0">
      </div>
      <div class="field">
        <label>Причина изменения</label>
        <input type="text" id="edit-order-reason" placeholder="напр. Скидка по договорённости">
      </div>
    </div>
    <div class="sheet-footer">
      <button class="btn-primary" onclick="submitEditOrderAmount(${orderId})">Сохранить</button>
    </div>
  `);
}

async function submitEditOrderAmount(orderId) {
  const newAmount = parseFloat(document.getElementById('edit-order-amount').value) || 0;
  const reason = document.getElementById('edit-order-reason').value.trim();
  if (!newAmount) { showToast('Укажи сумму', 'error'); return; }
  await withLoading(async () => {
    const { error } = await sb.from('orders').update({
    ...
    if (reason) {
      await sb.from('activity_logs').insert({
        action: 'order_amount_edited',
        entity_type: 'order',
        entity_id: orderId,
        details: { reason, new_amount: newAmount },
        created_by: S.user.id,
      });
    }
    closeSheet();
    showToast('Сумма обновлена ✓');
    switchFinancePeriod(_financePeriod);
  });
}

// ============================================
// ПРОИЗВОЛЬНЫЙ ПЕРИОД ДЛЯ AI РАСХОДОВ
// ============================================

let _aiCostsCustomFrom = '';
let _aiCostsCustomTo = '';

function showCustomPeriodAiCosts() {
  _aiCostsPeriod = 0;
  const c = document.getElementById('main-content');
  if (c) loadAiCostsContent(0).then(h => c.innerHTML = h);
}

function applyCustomPeriodAiCosts() {
  _aiCostsCustomFrom = document.getElementById('ai-date-from')?.value || '';
  _aiCostsCustomTo = document.getElementById('ai-date-to')?.value || '';
  if (!_aiCostsCustomFrom || !_aiCostsCustomTo) { showToast('Выбери обе даты', 'error'); return; }
  loadAiCostsContent(0).then(h => document.getElementById('main-content').innerHTML = h);
}

// ============================================
// ПРОИЗВОЛЬНЫЙ ПЕРИОД ДЛЯ МЕНЕДЖЕРОВ
// ============================================

let _managersCustomFrom = '';
let _managersCustomTo = '';

function showCustomPeriodManagers() {
  _managersPeriod = 0;
  const c = document.getElementById('main-content');
  if (c) loadManagersContent(0).then(h => c.innerHTML = h);
}

function applyCustomPeriodManagers() {
  _managersCustomFrom = document.getElementById('mgr-date-from')?.value || '';
  _managersCustomTo = document.getElementById('mgr-date-to')?.value || '';
  if (!_managersCustomFrom || !_managersCustomTo) { showToast('Выбери обе даты', 'error'); return; }
  loadManagersContent(0).then(h => document.getElementById('main-content').innerHTML = h);
}

async function openAssignManagerToOrderSheet(orderId) {
  const { data: order } = await sb.from('orders').select('deal_id, deals(assigned_to)').eq('id', orderId).single();
  const { data: managers } = await sb.from('profiles').select('id,full_name').eq('is_active', true);
  const currentId = order?.deals?.assigned_to || '';
  openSheet(`
    <div class="sheet-header"><h3>👤 Назначить менеджера</h3><button class="close-x" onclick="closeSheet()">✕</button></div>
    <div class="sheet-body">
      <div style="font-size:12.5px;color:var(--text-dim);margin-bottom:12px;">Выбери менеджера — его продажа учтётся в отчёте</div>
      ${(managers||[]).map(m => `
        <div class="card" style="display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;${m.id===currentId?'border-color:var(--accent);':''}" onclick="doAssignManagerToOrder(${orderId},'${m.id}')">
          <div style="flex:1;font-weight:600;">${escapeHtml(m.full_name||m.email)}</div>
          ${m.id===currentId?'<span style="color:var(--accent);">✓</span>':''}
        </div>
      `).join('')}
    </div>
  `);
...
async function doAssignManagerToOrder(orderId, managerId) {
  await withLoading(async () => {
    const { data: order } = await sb.from('orders').select('deal_id').eq('id', orderId).single();
    if (order?.deal_id) {
      const { error } = await sb.from('deals').update({ assigned_to: managerId }).eq('id', order.deal_id);
      if (error) throw error;
    }
    closeSheet();
    showToast('Менеджер назначен ✓');
