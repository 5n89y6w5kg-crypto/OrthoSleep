  ...
  openSheet(`
    ...
    <div class="sheet-body">
      ...
      <div class="card" style="background:${order.cost_paid ? 'rgba(0,185,86,0.08)' : 'rgba(232,162,58,0.08)'};border-color:${order.cost_paid ? 'var(--accent)' : 'var(--orange)'};">
        ...
        <div class="field"><label>Ширина матраса (см)</label>
...
function renderCalcResults(products) {
  ...
  const results = products.map(p => {
    ...
    // Себестоимость: cost_per_cm × ширина (или cost_price если нет per_cm)
    const costPerCm = p.cost_per_cm || (p.cost_price ? p.cost_price / 180 : 0);
    const costForWidth = costPerCm ? costPerCm * width : 0;
    const margin = discounted - costForWidth;
    const marginPct = discounted > 0 && costForWidth > 0 ? Math.round(margin / discounted * 100) : null;

    return { product: p, rawPrice, discounted, belowMin, exceedsMax, maxAllowed, costForWidth, margin, marginPct };
  ...
  return results.map(r => `
    <div class="calc-result-card ${r === cheapest ? 'best' : ''}">
      ...
      <div style="text-align:right;">
        ...
        <button style="margin-top:6px;background:var(--violet-bg);border:1px solid var(--violet);color:var(--violet);border-radius:8px;padding:5px 10px;font-size:11px;font-weight:700;" onclick="attachCalcResultToDeal(${r.product.id},'${escapeHtml(r.product.name).replace(/'/g,"\\'")}',${r.discounted.toFixed(2)},${width},${r.costForWidth.toFixed(2)})">В сделку →</button>
      </div>
    </div>
  `).join('');
}

function attachCalcResultToDeal(productId, productName, price, width, costForWidth = 0) {
  openSheet(`
    ...
    <div class="sheet-body">
      <div class="card">
        ...
        ${costForWidth > 0 ? `<div style="font-size:12px;color:var(--text-dim);margin-top:2px;">Себестоимость: ${fmtMoney(costForWidth)} · Прибыль: ${fmtMoney(price - costForWidth)}</div>` : ''}
      </div>
    ...
    <div class="sheet-footer">
      <button class="btn-primary" onclick="submitCalcDealAttach(${productId},'${escapeHtml(productName).replace(/'/g,"\\'")}',${price},${width},${costForWidth})">Создать сделку с этим товаром</button>
    </div>
  `);
}
