      ...
      <div class="card" style="text-align:center;padding:22px 16px;">
        ...
        ${client.instagram_username || client.facebook_id || client.whatsapp_number ? `
          ...
          <div class="theme-toggle" style="margin-top:10px;padding:10px 14px;" onclick="toggleClientAiManaged(${client.id},${client.ai_managed === false})">
            ...
            <div class="theme-switch" style="width:38px;height:22px;${client.ai_managed !== false ? 'background:rgba(0,185,86,0.15);' : ''}"><div class="knob" style="width:18px;height:18px;${client.ai_managed !== false ? 'left:18px;background:var(--accent);' : ''}"></div></div>
          </div>
        ` : ''}
      </div>
...
async function renderInbox() {
  ...
  for (const m of messages) {
    ...
    if (!byClient.has(m.client_id)) {
      byClient.set(m.client_id, { lastMessage: m, unreadCount: 0, needsHuman: false });
    }
...
async function renderAiAgentSection() {
  ...
  return `
    ...
    <div class="theme-toggle" onclick="toggleAiAgent(${!isOn})">
      ...
      <div class="theme-switch" style="${isOn ? 'background:var(--accent-bg, rgba(0,185,86,0.15));' : ''}"><div class="knob" style="${isOn ? 'left:26px;background:var(--accent);' : ''}"></div></div>
    </div>
    <div style="font-size:11.5px;color:var(--text-faint);margin:-4px 0 14px;padding:0 4px;">
      ${isOn ? 'AI сам отвечает в Instagram/Facebook, считает цены и создаёт сделки. Сложные случаи передаёт вам.' : 'Включи, чтобы AI начал отвечать клиентам автоматически.'}
    </div>
  `;
}
