...
async function renderTemplates() {
  ...
  return `
    <div class="page">
      <button class="btn-primary" style="margin-bottom:14px;" onclick="openCreateTemplateSheet()">+ Новый шаблон</button>
      ${Object.entries(grouped).map(([cat, items]) => `
        <div class="section-title">${cats[cat] || cat}</div>
        ${items.map(t => `
          <div class="card" style="padding:12px 14px;">
            <div style="font-weight:600;font-size:13.5px;margin-bottom:6px;">${escapeHtml(t.title)}</div>
            <div style="font-size:12.5px;color:var(--text-dim);line-height:1.5;margin-bottom:10px;">${escapeHtml(t.content)}</div>
            <div style="display:flex;gap:8px;">
              <button class="btn-secondary" style="font-size:11.5px;" onclick="copyTemplate('${escapeHtml(t.content).replace(/'/g,"\\'")}')">📋 Копировать</button>
              <button class="btn-secondary" style="font-size:11.5px;" onclick="editTemplate(${t.id})">✏️</button>
              <button class="btn-secondary" style="font-size:11.5px;color:var(--red);" onclick="deleteTemplate(${t.id})">🗑️</button>
            </div>
          </div>
        `).join('')}
      `).join('')}
