...
function renderTopbar() {
  ...
  const showBack = ['client_detail', 'deal_detail', 'tasks', 'analytics', 'inbox', 'ai_knowledge', 'activity_logs', 'notifications', 'finance', 'conversation_analysis', 'templates', 'health', 'ai_costs', 'dialog_export'].includes(S.route);
      ...
      ${rows.length ? rows.map(o => `
        <div class="card" style="font-size:13px;">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;color:var(--text-dim);font-size:11.5px;flex-wrap:wrap;gap:4px;">
            <span>Себест: <b>${fmtMoney(o.cost)}</b></span>
          ...
          ${!o.cost_paid && o.cost > 0 ? `
            <button class="btn-secondary" style="margin-top:8px;font-size:11.5px;padding:5px 10px;" onclick="openOrderDetailSheet(${o.id})">Отметить оплату поставщику →</button>
          ` : ''}
        </div>
...
async function loadAiCostsContent(days, initialBalance) {
  ...
  if (initialBalance === undefined) {
    initialBalance = parseFloat(localStorage.getItem('ai_initial_balance') || '0');
  }
