  ...
  openSheet(`
    ...
    <div class="sheet-body">
      ...
      <div class="card" style="background:${order.cost_paid ? 'rgba(0,185,86,0.08)' : 'rgba(232,162,58,0.08)'};border-color:${order.cost_paid ? 'var(--accent)' : 'var(--orange)'};">
        ...
        <div class="field"><label>Сумма себестоимости (сом)</label><input type="number" id="order-cost-amount" value="${order.cost_paid_amount || ''}" placeholder="Введи сумму которую заплатил поставщику"></div>
        <div class="field"><label>Примечание (необязательно)</label><input type="text" id="order-cost-notes" value="${escapeHtml(order.cost_notes || '')}" placeholder="напр. оплата наличными, перевод"></div>
        <div class="btn-row">
