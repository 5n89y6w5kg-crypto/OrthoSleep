...
function renderTopbar() {
  ...
  const showBack = ['client_detail', 'deal_detail'].includes(S.route);
...
async function renderPipeline() {
  ...
  return `<div class="kanban-scroll">${columns}</div>`;
}
...
async function renderClientDetail(id) {
  ...
  return `
    <div class="page">
      ...
      <button class="btn-secondary" style="margin-top:4px;margin-bottom:18px;" onclick="prefillDealForClient(${client.id},'${escapeHtml(client.full_name).replace(/'/g,"\\'")}')">+ Новая сделка для этого клиента</button>
  ...
  return `
    <div class="page">
      <div class="card">
        ...
        ${deal.clients.phone ? `<a href="tel:${deal.clients.phone}" class="btn-secondary" style="display:block;text-align:center;margin-top:12px;">📞 ${escapeHtml(deal.clients.phone)}</a>` : ''}
      </div>
...
async function renderSettings() {
  ...
  if (isAdmin) {
    managersHtml = `
      ...
      `).join('')}
      <div style="font-size:12px;color:var(--text-faint);margin:6px 0 18px;">Чтобы добавить менеджера — создай пользователя в Supabase (Authentication → Users), он появится здесь автоматически.</div>
    `;
  }
  ...
  return `
    <div class="page">
      ...
      <div class="card" style="font-size:13px;color:var(--text-dim);line-height:1.6;">
        🛏️ OrthoSleep CRM<br>
        Версия 1.0 · Этап 1 (фундамент)<br>
