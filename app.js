  ...
  return items.map(it => {
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border);">
      <div>
        ...
        <div style="font-size:12px;color:var(--text-dim);">${it.qty} × ${fmtMoney(it.price)}${S.profile?.role === 'admin' && it.cost_price_at_sale ? ` · себест: ${fmtMoney(it.cost_price_at_sale)}` : ''}</div>
      </div>
...
function onAddItemSizeChange() {
  ...
  if (ppc > 0) {
    ...
    resultDiv.innerHTML = `Цена: <b>${fmtMoney(calcPrice)}</b>${S.profile?.role === 'admin' && calcCost > 0 ? ` · Себест: <b>${fmtMoney(calcCost)}</b>` : ''}${S.profile?.role === 'admin' && profit !== null ? ` · Прибыль: <b style="color:var(--accent)">${fmtMoney(profit)}</b>` : ''}`;
        ...
        <div class="product-card">
          ...
          <div class="prow">
            <div>
              ...
              ${S.profile?.role === 'admin' ? (p.cost_price ? `<div style="font-size:11px;color:var(--text-faint);">Себест: ${fmtMoney(p.cost_price)}</div>` : `<div style="font-size:11px;color:var(--orange);">⚠️ Нет себестоимости</div>`) : ''}
    ...
    <div class="page">
      ...
      ${filtered.map(o => {
        ...
        <div class="card">
          ...
          <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;">
            ${isAdmin ? `<button class="btn-secondary" style="font-size:11px;padding:5px 8px;" onclick="openEditOrderAmountSheet(${o.id}, ${o.total_amount})">✏️ Сумма</button>` : ''}
            ${isAdmin ? `<button class="btn-secondary" style="font-size:11px;padding:5px 8px;" onclick="openAssignManagerToOrderSheet(${o.id})">👤 Менеджер</button>` : ''}
          </div>
        </div>
      ...
      <div class="calc-input-card">
        ...
        <div style="margin-top:10px;display:flex;gap:8px;">
          <div style="flex:1;">
            ...
            <input type="number" id="calc-custom-discount" placeholder="напр. 12" value="${[0,10,15,20].includes(_calcState.discountPercent) ? '' : _calcState.discountPercent}" oninput="setCalcDiscountByPct(parseFloat(this.value)||0)">
          </div>
          <div style="flex:1;">
            <div style="font-size:11px;color:var(--text-faint);margin-bottom:4px;">Итоговая сумма (сом)</div>
            <input type="number" id="calc-final-amount" placeholder="напр. 2200" oninput="setCalcDiscountByFinalSum(parseFloat(this.value)||0)">
          </div>
        </div>
      </div>
...
function setCalcDiscount(percent) {
  _calcState.discountPercent = percent;
  const pctEl = document.getElementById('calc-custom-discount');
  if (pctEl) pctEl.value = [0,10,15,20].includes(percent) ? '' : (percent||'');
  _syncCalcFinalAmount();
  recalcCalculator();
}
...
function setCalcDiscountByFinalSum(finalSum) {
  const w = _calcState.width || 180;
  const first = (S.cache.allProducts||[]).find(p => p.price_per_cm);
  if (!first || !finalSum) return;
  const base = first.price_per_cm * w;
  const pct = Math.max(0, Math.round((1 - finalSum / base) * 1000) / 10);
  _calcState.discountPercent = pct;
  const pctEl = document.getElementById('calc-custom-discount');
  if (pctEl) pctEl.value = pct > 0 ? pct : '';
  recalcCalculator();
}

function _syncCalcFinalAmount() {
  const el = document.getElementById('calc-final-amount');
  if (!el) return;
  const w = _calcState.width || 180;
  const first = (S.cache.allProducts||[]).find(p => p.price_per_cm);
  if (first && _calcState.discountPercent > 0) {
    el.value = Math.round(first.price_per_cm * w * (1 - _calcState.discountPercent / 100));
  } else {
    el.value = '';
  }
}
  ...
  return results.map(r => `
    <div class="calc-result-card ${r === cheapest ? 'best' : ''}">
      <div>
        ...
        ${S.profile?.role === 'admin' && r.costForWidth > 0 ? `
  ...
  openSheet(`
    ...
    <div class="sheet-body">
      <div class="card">
        ...
        ${S.profile?.role === 'admin' && costForWidth > 0 ? `<div style="font-size:12px;color:var(--text-dim);margin-top:2px;">Себестоимость: ${fmtMoney(costForWidth)} · Прибыль: ${fmtMoney(Math.round(price - costForWidth))}</div>` : ''}
      </div>
...
function generateCSV(headers, rows, filename) {
  try {
    ...
    const wsData = [headers, ...rows.map(r => r.map(v => (v === null || v === undefined) ? '' : v))];
    const ws = XLSX.utils.aoa_to_sheet(wsData);
    ws['!cols'] = headers.map((h, i) => ({ wch: Math.max(h.length, 15) }));
    XLSX.utils.book_append_sheet(wb, ws, 'Данные');
    XLSX.writeFile(wb, filename.replace(/\.tsv|\.csv/, '.xlsx'));
    showToast('Excel файл скачан ✓');
  } catch(e) {
    // Fallback TSV если SheetJS не загружен
    const lines = [headers.join('\t'), ...rows.map(r =>
      r.map(v => String(v === null || v === undefined ? '' : v).replace(/[\t\n\r]/g, ' ')).join('\t')
    )];
    const blob = new Blob(['\uFEFF' + lines.join('\r\n')], { type: 'text/tab-separated-values;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = filename.replace(/\.xlsx/, '.tsv');
