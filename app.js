      ...
      ${orders.length ? orders.map(o => {
        ...
        <div class="card" onclick="openOrderDetailSheet(${o.id})">
          ...
          <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;">
            <span class="badge" style="background:${o.payment_status==='paid'?'rgba(0,185,86,0.15)':o.payment_status==='partial'?'rgba(232,162,58,0.15)':'rgba(220,50,50,0.15)'};color:${o.payment_status==='paid'?'var(--accent)':o.payment_status==='partial'?'var(--orange)':'var(--red)'};">${payLabels[o.payment_status]?.[0] || o.payment_status}</span>
            <span class="badge" style="background:rgba(0,0,0,0.08);color:var(--text-dim);">${delLabels[o.delivery_status]?.[0] || o.delivery_status}</span>
            <span class="badge" style="background:${o.cost_paid?'rgba(0,185,86,0.12)':'rgba(232,162,58,0.12)'};color:${o.cost_paid?'var(--accent)':'var(--orange)'};">${o.cost_paid?'✅ Поставщик':'⏳ Поставщик'}</span>
          </div>
        </div>
...
async function downloadOrdersCsv() {
  try {
    ...
    const headers = ['№ заказа', 'Клиент', 'Телефон', 'Сделка', 'Сумма', 'Оплачено', 'Остаток', 'Статус оплаты', 'Статус доставки', 'Дата создания', 'Дата доставки'];
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `orthosleep-orders-${new Date().toISOString().slice(0,10)}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    ...
    showToast(err.message || 'Ошибка экспорта заказов', 'error');
  }
}
