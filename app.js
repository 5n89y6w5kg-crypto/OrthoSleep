...
function renderTopbar() {
  ...
  const showBack = ['client_detail', 'deal_detail', 'tasks', 'analytics', 'inbox'].includes(S.route);
  ...
  return `
    <div class="page">
      <div class="card" style="text-align:center;padding:22px 16px;">
        ...
        ${client.instagram_username ? `<button class="btn-primary" style="margin-top:10px;" onclick="openMessagesSheet(${client.id})">💬 Открыть переписку</button>` : ''}
      </div>
      ...
      ${history.length ? history.map(h => `
        <div class="timeline-item">
          <div class="timeline-dot" style="background:${h.type === 'system' || h.type === 'status_change' ? 'var(--blue)' : 'var(--gold)'}"></div>
          <div>
...
async function renderSettings() {
  ...
  return `
    <div class="page">
      ...
      ${S.profile.role === 'admin' ? `
        <div class="section-title">🔧 Диагностика Instagram</div>
        <div class="card">
          <button class="btn-secondary" style="margin-bottom:8px;" onclick="checkInstagramSubscription()">Проверить подписку</button>
          <button class="btn-secondary" onclick="subscribeInstagram()">Подписаться на сообщения</button>
...
async function checkInstagramSubscription() {
  const box = document.getElementById('diag-result');
  box.textContent = 'Загрузка...';
  try {
    const { data: sessionData } = await sb.auth.getSession();
    const token = sessionData.session?.access_token;
    const res = await fetch(`${SUPABASE_URL}/functions/v1/instagram-webhook?action=check_subscription`, {
      method: 'POST',
...
async function subscribeInstagram() {
  const box = document.getElementById('diag-result');
  box.textContent = 'Подписываемся...';
  try {
    const { data: sessionData } = await sb.auth.getSession();
    const token = sessionData.session?.access_token;
    const res = await fetch(`${SUPABASE_URL}/functions/v1/instagram-webhook?action=subscribe`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'apikey': SUPABASE_ANON_KEY, 'Content-Type': 'application/json' },
    });
    const data = await res.json();
    box.textContent = JSON.stringify(data, null, 2);
    if (data.success) showToast('Подписка обновлена');
  } catch (err) {
    box.textContent = 'Ошибка: ' + err.message;
  }
}
...
async function openMessagesSheet(clientId) {
  ...
  const client = (await sb.from('clients').select('full_name,photo_url').eq('id', clientId).single()).data;
...
async function loadChatMessages(clientId, silent = false) {
  ...
  if (unreadIds.length) {
    sb.from('messages').update({ is_read: true }).in('id', unreadIds);
  }
}
...
async function sendChatMessage() {
  ...
  try {
    ...
    const res = await fetch(`${SUPABASE_URL}/functions/v1/instagram-webhook?action=send`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
...
async function renderInbox() {
  ...
  const { data: messages, error } = await sb
    .from('messages')
    .select('*, clients(id,full_name,photo_url,instagram_username,whatsapp_number,facebook_id), channels(code,name)')
    .order('created_at', { ascending: false })
    .limit(300);
  if (error) throw error;
  ...
  for (const m of messages) {
    ...
    if (!byClient.has(m.client_id)) {
      byClient.set(m.client_id, { lastMessage: m, unreadCount: 0 });
    }
  ...
  return `
    <div class="page" style="padding-top:8px;">
      ${conversations.map(({ lastMessage: m, unreadCount }) => `
        <div class="client-row" onclick="openMessagesSheet(${m.clients.id})" style="${unreadCount > 0 ? 'border-color:var(--violet);' : ''}">
          ...
          <div class="info">
            ...
            <div class="meta" style="display:block;">
              <span style="color:${unreadCount > 0 ? 'var(--text)' : 'var(--text-dim)'};font-weight:${unreadCount > 0 ? '600' : '400'};">${m.direction === 'outbound' ? 'Вы: ' : ''}${escapeHtml((m.content || '').slice(0, 45))}${(m.content || '').length > 45 ? '…' : ''}</span>
            </div>
          </div>
